<div class="front_page_section front_page_section_team<?php
	$woohoo_scheme = woohoo_get_theme_option( 'front_page_team_scheme' );
	if ( ! empty( $woohoo_scheme ) && ! woohoo_is_inherit( $woohoo_scheme ) ) {
		echo ' scheme_' . esc_attr( $woohoo_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( woohoo_get_theme_option( 'front_page_team_paddings' ) );
	if ( woohoo_get_theme_option( 'front_page_team_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$woohoo_css      = '';
		$woohoo_bg_image = woohoo_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$woohoo_anchor_icon = woohoo_get_theme_option( 'front_page_team_anchor_icon' );
	$woohoo_anchor_text = woohoo_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( woohoo_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$woohoo_css      = '';
			$woohoo_bg_mask  = woohoo_get_theme_option( 'front_page_team_bg_mask' );
			$woohoo_bg_color_type = woohoo_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $woohoo_bg_color_type ) {
				$woohoo_bg_color = woohoo_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$woohoo_caption = woohoo_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $woohoo_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $woohoo_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $woohoo_caption, 'woohoo_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$woohoo_description = woohoo_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $woohoo_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $woohoo_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $woohoo_description ), 'woohoo_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_team_output">
				...
				if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
					...
					if ( ! woohoo_exists_trx_addons() ) {
						...
						woohoo_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
					}
				}
				?>
			</div>
