...
jQuery( document ).ready(
	function() {
		...
		woohoo_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="woohoo_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.woohoo_post_editor' );
						if ( link.length > 0 ) {
							woohoo_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.woohoo_post_editor', function(e) {
						woohoo_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('woohoo_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function woohoo_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
