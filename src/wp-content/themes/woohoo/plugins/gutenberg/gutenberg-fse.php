...
if ( ! defined( 'WOOHOO_FSE_TEMPLATE_PART_PT' ) ) define( 'WOOHOO_FSE_TEMPLATE_PART_PT', 'wp_template_part' );

if ( ! defined( 'WOOHOO_FSE_TEMPLATE_PART_AREA_HEADER' ) )  define( 'WOOHOO_FSE_TEMPLATE_PART_AREA_HEADER',  defined( 'WP_TEMPLATE_PART_AREA_HEADER' )  ? WP_TEMPLATE_PART_AREA_HEADER  : 'header' );
if ( ! defined( 'WOOHOO_FSE_TEMPLATE_PART_AREA_FOOTER' ) )  define( 'WOOHOO_FSE_TEMPLATE_PART_AREA_FOOTER',  defined( 'WP_TEMPLATE_PART_AREA_FOOTER' )  ? WP_TEMPLATE_PART_AREA_FOOTER  : 'footer' );
if ( ! defined( 'WOOHOO_FSE_TEMPLATE_PART_AREA_SIDEBAR' ) ) define( 'WOOHOO_FSE_TEMPLATE_PART_AREA_SIDEBAR', defined( 'WP_TEMPLATE_PART_AREA_SIDEBAR' ) ? WP_TEMPLATE_PART_AREA_SIDEBAR : 'sidebar' );
if ( ! defined( 'WOOHOO_FSE_TEMPLATE_PART_AREA_UNCATEGORIZED' ) ) define( 'WOOHOO_FSE_TEMPLATE_PART_AREA_UNCATEGORIZED', defined( 'WP_TEMPLATE_PART_AREA_UNCATEGORIZED' ) ? WP_TEMPLATE_PART_AREA_UNCATEGORIZED : 'uncategorized' );

// Load additional files with FSE support
require_once woohoo_get_file_dir( 'plugins/gutenberg/gutenberg-fse-lists.php' );
require_once woohoo_get_file_dir( 'plugins/gutenberg/gutenberg-fse-templates.php' );


if ( ! function_exists( 'woohoo_gutenberg_fse_loader' ) ) {
	add_action( 'wp_loaded', 'woohoo_gutenberg_fse_loader', 1 );
	/**
	 * Turn off a Gutenberg (plugin) templates on frontend
	 * if a theme-specific frontend editor is active.
	 */
	function woohoo_gutenberg_fse_loader() {
		// 
		if ( woohoo_exists_gutenberg_fse()
			&& get_option( 'show_on_front' ) == 'page'
			&& (int) get_option('page_on_front') > 0
			&& woohoo_is_on( woohoo_get_theme_option( 'front_page_enabled', false ) )
			&& woohoo_get_current_url() == '/'
		) {
...
if ( ! function_exists( 'woohoo_gutenberg_is_fse_wp_support' ) ) {
	/**
	 * Check if WordPress with FSE support installed
	 * (WordPress 5.9+ Site Editor is present).
	 */
	function woohoo_gutenberg_is_fse_wp_support() {
		return function_exists( 'wp_is_block_theme' );
	}
}

if ( ! function_exists( 'woohoo_gutenberg_is_fse_gb_support' ) ) {
	/**
	 * Check if a plugin "Gutenberg" with FSE support is installed.
	 */
	function woohoo_gutenberg_is_fse_gb_support() {
		return function_exists( 'gutenberg_is_fse_theme' );
	}
}

if ( ! function_exists( 'woohoo_exists_gutenberg_fse' ) ) {
	/**
	 * Check if FSE is available
	 * (WordPress 5.9+ or the plugin "Gutenberg" are installed).
	 */
	function woohoo_exists_gutenberg_fse() {
		return woohoo_exists_gutenberg() && ( woohoo_gutenberg_is_fse_wp_support() || woohoo_gutenberg_is_fse_gb_support() );
	}
}

if ( ! function_exists( 'woohoo_gutenberg_is_fse_theme' ) ) {
	/**
	 * Check if the current theme supports a FSE behaviour.
	 */
	function woohoo_gutenberg_is_fse_theme() {
		static $fse = -1;
		if ( $fse == -1 ) {
			if ( woohoo_gutenberg_is_fse_wp_support() ) {                                    // WordPress 5.9+ FSE
				$fse = (bool) wp_is_block_theme();
			} else if ( woohoo_gutenberg_is_fse_gb_support() ) {                             // Plugin Gutenberg FSE
				$fse = (bool) gutenberg_is_fse_theme();
			} else {
				$fse = is_readable( WOOHOO_THEME_DIR . 'templates/index.html' )              // WordPress 5.9+ FSE
						|| ( WOOHOO_THEME_DIR != WOOHOO_CHILD_DIR && is_readable( WOOHOO_CHILD_DIR . 'templates/index.html' ) )
						|| is_readable( WOOHOO_THEME_DIR . 'block-templates/index.html' )    // Plugin Gutenberg FSE
						|| ( WOOHOO_THEME_DIR != WOOHOO_CHILD_DIR && is_readable( WOOHOO_CHILD_DIR . 'block-templates/index.html' ) );
			}
		}
		return $fse;
	}
...
if ( ! function_exists( 'woohoo_gutenberg_is_fse_enabled' ) ) {
	/**
	 * Check if the current theme supports FSE behaviour and FSE is installed and activated
	 */
	function woohoo_gutenberg_is_fse_enabled() {
		return woohoo_exists_gutenberg_fse() && woohoo_gutenberg_is_fse_theme();
	}
}

if ( ! function_exists( 'woohoo_gutenberg_fse_skin_updated' ) ) {
	add_action( 'woohoo_action_theme_updated', 'woohoo_gutenberg_fse_skin_updated' );
	add_action( 'woohoo_action_skin_updated', 'woohoo_gutenberg_fse_skin_updated', 10, 1 );
	/**
	 * Copy FSE-required folders from an updated skin's subfolder '_fse' to the theme's root folder.
	 * 
	 * Hooks: add_action( 'woohoo_action_skin_updated', 'woohoo_gutenberg_fse_skin_updated', 10, 1 );
	 */
	function woohoo_gutenberg_fse_skin_updated( $skin = '' ) {
		// Get an active skin name if empty
		if ( current_action() == 'woohoo_action_theme_updated' && empty( $skin ) ) {
			$skin = woohoo_skins_get_active_skin_name();
		}
		if ( woohoo_skins_get_current_skin_name() == $skin ) {
			woohoo_gutenberg_fse_copy_from_skin( $skin );
			woohoo_gutenberg_fse_update_theme_json( $skin );
		}
	}
}

if ( ! function_exists( 'woohoo_gutenberg_fse_copy_from_skin' ) ) {
	add_action( 'woohoo_action_skin_switched', 'woohoo_gutenberg_fse_copy_from_skin', 10, 2 );
	/**
	 * Copy FSE-required folders from a new skin's subfolder '_fse' to the theme's root folder.
	 * 
	 * Hooks: add_action( 'woohoo_action_skin_switched', 'woohoo_gutenberg_fse_copy_from_skin', 10, 2 );
	 */
	function woohoo_gutenberg_fse_copy_from_skin( $new_skin, $old_skin = '' ) {
		$theme_templates_dir = woohoo_prepare_path( WOOHOO_THEME_DIR . ( woohoo_gutenberg_is_fse_wp_support() ? 'templates/' : 'block-templates/' ) );
		$theme_template_parts_dir = woohoo_prepare_path( WOOHOO_THEME_DIR . ( woohoo_gutenberg_is_fse_wp_support() ? 'parts/' : 'block-template-parts/' ) );
		$theme_json = woohoo_prepare_path( WOOHOO_THEME_DIR . 'theme.json' );
		$skin_templates_dir = woohoo_prepare_path( WOOHOO_THEME_DIR . 'skins/' . $new_skin . '/_fse' . ( woohoo_gutenberg_is_fse_wp_support() ? '/templates/' : '/block-templates/' ) );
		$skin_template_parts_dir = woohoo_prepare_path( WOOHOO_THEME_DIR . 'skins/' . $new_skin . '/_fse' . ( woohoo_gutenberg_is_fse_wp_support() ? '/parts/' : '/block-template-parts/' ) );
		// Remove old templates from the stylesheet dir (if exists)
		if ( is_dir( $theme_templates_dir ) ) {
			woohoo_unlink( $theme_templates_dir );
		}
		if ( is_dir( $theme_template_parts_dir ) ) {
			woohoo_unlink( $theme_template_parts_dir );
		}
		if ( file_exists( $theme_json ) ) {
			woohoo_unlink( $theme_json );
		}
		...
		if ( is_dir( $skin_templates_dir ) ) {
			woohoo_copy( $skin_templates_dir, $theme_templates_dir );
		}
		if ( is_dir( $skin_template_parts_dir ) ) {
			woohoo_copy( $skin_template_parts_dir, $theme_template_parts_dir );
		}
	}
}

if ( ! function_exists( 'woohoo_gutenberg_fse_update_theme_json' ) ) {
	add_action( 'woohoo_action_skin_switched', 'woohoo_gutenberg_fse_update_theme_json', 30, 2 );
	add_action( 'woohoo_action_save_options', 'woohoo_gutenberg_fse_update_theme_json', 30 );
	add_action( 'trx_addons_action_save_options', 'woohoo_gutenberg_fse_update_theme_json', 30 );
	/**
	 * Update the file 'theme.json' after the current skin is switched and/or theme options are saved.
	 * 
	 * Hooks: add_action( 'woohoo_action_skin_switched', 'woohoo_gutenberg_fse_update_theme_json', 30, 2 );
	 * 
	 *        add_action( 'woohoo_action_save_options', 'woohoo_gutenberg_fse_update_theme_json', 30 );
	 * 
	 *        add_action( 'trx_addons_action_save_options', 'woohoo_gutenberg_fse_update_theme_json', 30 );
	 * 
	 * Trigger filter 'woohoo_filter_gutenberg_fse_theme_json_data' to allow other modules modify this data before saving.
	 */
	function woohoo_gutenberg_fse_update_theme_json( $new_skin = '', $old_skin = '' ) {
		// Get an active skin name if empty
		if ( in_array( current_action(), array( 'woohoo_action_save_options', 'trx_addons_action_save_options' ) ) && empty( $new_skin ) ) {
			$new_skin = woohoo_skins_get_active_skin_name();
		}
		// If the current skin supports FSE
		if ( ! empty( $new_skin ) && is_dir( WOOHOO_THEME_DIR . woohoo_skins_get_current_skin_dir( $new_skin ) . '_fse' ) ) {
			$theme_json_data = woohoo_gutenberg_fse_theme_json_data();
			if ( ! empty( $theme_json_data ) && is_array( $theme_json_data ) ) {
				woohoo_fpc( woohoo_prepare_path( WOOHOO_THEME_DIR . 'theme.json' ), json_encode( $theme_json_data, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ) );
			}
		}
	}
}

if ( ! function_exists( 'woohoo_gutenberg_fse_theme_json_data' ) ) {
	/**
	 * Return a default data to save to the file 'theme.json'
	 * 
	 * Trigger filter 'woohoo_filter_gutenberg_fse_theme_json_data' to allow other modules change data.
	 * 
	 * @return array  Data for the 'theme.json'
	 */
	function woohoo_gutenberg_fse_theme_json_data() {
		...
		$data = array(
					...
					"customTemplates" => array(
						array(
							"name"      => "blog",
							"title"     => esc_html__( "Blog archive", 'woohoo' ),
							"postTypes" => array(
								"page"
							)
						)
					...
					"templateParts" => array(
						array(
							"name"  => "header",
							"title" => esc_html__( "Header for FSE", 'woohoo' ),
							"area"  => WOOHOO_FSE_TEMPLATE_PART_AREA_HEADER,
						),
						array(
							"name"  => "sidebar",
							"title" => esc_html__( "Sidebar for FSE", 'woohoo' ),
							"area"  => WOOHOO_FSE_TEMPLATE_PART_AREA_SIDEBAR,
						),
						array(
							"name"  => "content-404",
							"title" => esc_html__( "Page 404 content", 'woohoo' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "none-archive",
							"title" => esc_html__( "No posts found", 'woohoo' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "none-search",
							"title" => esc_html__( "Nothing matched search", 'woohoo' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "blog-post-standard",
							"title" => esc_html__( "Blog post item: Standard", 'woohoo' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "blog-post-header",
							"title" => esc_html__( "Blog post header", 'woohoo' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "blog-pagination",
							"title" => esc_html__( "Blog pagination", 'woohoo' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "footer",
							"title" => esc_html__( "Footer for FSE", 'woohoo' ),
...
*/
		...
		if ( ! empty( $scheme['bg_color'] ) && ! empty( $scheme['text'] ) ) {
			$data['settings']['color']['duotone'][] = array(
				...
				"slug"   => 'bg-and-text',
				"name"   => esc_html__( 'Background and text color', 'woohoo' )
			);
		}
		if ( ! empty( $scheme['bg_color'] ) && ! empty( $scheme['text_dark'] ) ) {
			$data['settings']['color']['duotone'][] = array(
				...
				"slug"   => 'bg-and-dark',
				"name"   => esc_html__( 'Background and dark color', 'woohoo' )
			);
		}
		if ( ! empty( $scheme['bg_color'] ) && ! empty( $scheme['text_link'] ) ) {
			$data['settings']['color']['duotone'][] = array(
				...
				"slug"   => 'bg-and-link',
				"name"   => esc_html__( 'Background and link color', 'woohoo' )
			);
		}
		...
		if ( ! empty( $scheme['text_hover'] ) && ! empty( $scheme['text_link'] ) ) {
			$data['settings']['color']['gradients'][] = array(
				...
				"name"     => esc_html__( 'Vertical from link color to hover color', 'woohoo' ),
			);
			$data['settings']['color']['gradients'][] = array(
				...
				"name"     => esc_html__( 'Diagonal from link color to hover color', 'woohoo' ),
			);
		}
/*
		// Add fonts
...
*/
		...
		return apply_filters( 'woohoo_filter_gutenberg_fse_theme_json_data', $data );
	}
}
