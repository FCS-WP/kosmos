...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			woohoo_create_observer( 'woohoo-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				woohoo_gutenberg_editor_init();
			} );
		} else {
		 	woohoo_gutenberg_editor_init();
		}
	...
	function woohoo_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'woohoo_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'woohoo_inited' ) ) {
	...
	// Init on page load
	function woohoo_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = woohoo_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + WOOHOO_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + WOOHOO_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in WOOHOO_STORAGE['override_classes'] ) {
				$obj.addClass( WOOHOO_STORAGE['override_classes'][i].replace( '%s', WOOHOO_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( woohoo_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && WOOHOO_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'woohoo_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			woohoo_remove_observer( 'woohoo-check-editor-styles-wrapper' );
			woohoo_create_observer( 'woohoo-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'woohoo_inited' );
	}
} );
