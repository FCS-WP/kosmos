...
if ( ! function_exists( 'woohoo_gutenberg_fse_replace_frontpage_content' ) ) {
	add_filter( 'frontpage_template', 'woohoo_gutenberg_fse_replace_frontpage_content', 9999, 3 );
	/**
	 ...
	 * 
	 * Hooks: add_filter( 'frontpage_template', 'woohoo_gutenberg_fse_replace_frontpage_content', 9999, 3 );
	 *
	 * @param string   $template  Path to the template. See locate_template().
	 * @param string   $type      Sanitized filename without extension.
	 * @param string[] $templates A list of template candidates, in descending order of priority.
	 */
	function woohoo_gutenberg_fse_replace_frontpage_content( $template, $type = '', $templates = array() ) {
...
if ( ! function_exists( 'woohoo_gutenberg_fse_replace_no_posts_content' ) ) {
	add_filter( 'search_template',  'woohoo_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	add_filter( 'index_template',   'woohoo_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	add_filter( 'archive_template', 'woohoo_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	/**
	 * Substitute a content of the global variable $_wp_current_template_content
	 * with a content of the template 'parts/none-archive.html' or 'templates/none-archive.html'.
	 * 
	 * Hooks: add_filter( 'search_template',  'woohoo_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	 * 
	 *        add_filter( 'index_template',   'woohoo_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	 * 
	 *        add_filter( 'archive_template', 'woohoo_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	 ...
	 * @param string[] $templates A list of template candidates, in descending order of priority.
	 */
	function woohoo_gutenberg_fse_replace_no_posts_content( $template, $type = '', $templates = array() ) {
		if ( substr( $template, -19 ) == 'template-canvas.php' ) {
			if ( ! have_posts() ) {
				...
				$part_exists = false;
				$data = woohoo_gutenberg_fse_theme_json_data();
				...
				$no_posts_file = woohoo_get_file_dir( ( $part_exists ? 'parts/' : 'templates/' ) . "{$part_name}.html" );
				if ( ! empty( $no_posts_file ) ) {
					// Check if exists a saved template with a same name
					$posts = get_posts( array(
						'name'           => $part_name,
						'posts_per_page' => 1,
						'post_type'      => defined( 'WOOHOO_FSE_TEMPLATE_PART_PT' ) ? WOOHOO_FSE_TEMPLATE_PART_PT : 'wp_template_part',
					) );
					// Get a template content from the database or from the file (if a saved version is not found)
					$content = ! empty( $posts[0]->post_content ) ? $posts[0]->post_content : woohoo_fgc( $no_posts_file );
...
if ( ! function_exists( 'woohoo_gutenberg_fse_show_template_as_layout' ) ) {
	add_filter( 'woohoo_filter_custom_layout_shown', 'woohoo_gutenberg_fse_show_template_as_layout', 10, 3 );
	/**
	 * Show an FSE template (header/footer/sidebar) inside a non-FSE templates.
	 * 
	 * Hook: add_filter( 'woohoo_filter_custom_layout_shown', 'woohoo_gutenberg_fse_show_template_as_layout', 10, 3 );
	 * 
	 ...
	 */
	function woohoo_gutenberg_fse_show_template_as_layout( $shown, $layout_id = '', $post_id = 0 ) {
		// If a layout_id is a string with a template part name
		if ( ! $shown && ! empty( $layout_id ) && is_string( $layout_id) && (int)$layout_id === 0 && woohoo_gutenberg_is_fse_theme() ) {
			// And file with template-name.html is present in the theme subfolder '/parts'
			$template_path = woohoo_get_file_dir( "/parts/{$layout_id}.html" );
			if ( ! empty( $template_path ) ) {
				$template_content = woohoo_fgc( $template_path );
				if ( ! empty( $template_content ) ) {
					woohoo_show_layout( woohoo_filter_post_content( $template_content ) );
					$shown = true;
				}
			}
		}
...
if ( ! function_exists( 'woohoo_gutenberg_fse_modify_template_with_current_options' ) ) {
	foreach( woohoo_get_wp_template_hooks() as $hook ) {
		add_filter( $hook, 'woohoo_gutenberg_fse_modify_template_with_current_options', 30, 3 );
	}
	/**
	 ...
	 * 
	 * Trigger the filter 'woohoo_filter_wp_current_template_content' to allow other modules
	 ...
	 */
	function woohoo_gutenberg_fse_modify_template_with_current_options( $template, $type = '', $templates = array() ) {
		global $_wp_current_template_content;
		if ( substr( $template, -19 ) == 'template-canvas.php' && is_array( $templates ) ) {
			$_wp_current_template_content = apply_filters( 'woohoo_filter_wp_current_template_content', $_wp_current_template_content, $template, $type, $templates );
		}
		return $template;
	}
}

if ( ! function_exists( 'woohoo_gutenberg_fse_modify_template_replace_macros' ) ) {
//	add_filter( 'woohoo_filter_wp_current_template_content', 'woohoo_gutenberg_fse_modify_template_replace_macros', 100, 4 );
	/**
	 * Replace a macros {{Y}} and {Y} in the content of the current template to the current year.
	 * 
	 * Hooks: add_filter( 'woohoo_filter_wp_current_template_content', 'woohoo_gutenberg_fse_modify_template_replace_macros', 10, 4 );
	 *
	 ...
	 */
	function woohoo_gutenberg_fse_modify_template_replace_macros( $content, $template, $type, $templates ) {
...
// Header
//----------------------------------------------
if ( ! function_exists( 'woohoo_gutenberg_fse_modify_template_replace_header' ) ) {
	add_filter( 'woohoo_filter_wp_current_template_content', 'woohoo_gutenberg_fse_modify_template_replace_header', 10, 4 );
	/**
	 * Replace a header in the content of the current template
	 * according to the Theme Options of the current page.
	 * 
	 * Hooks: add_filter( 'woohoo_filter_wp_current_template_content', 'woohoo_gutenberg_fse_modify_template_replace_header', 10, 4 );
	 *
	 * @param string $content    A content of the current template.
	 * @param string $template   Path to the template. See locate_template().
	 * @param string $type       Sanitized filename without extension.
	 ...
	 * @return string  A filtered content of the template.
	 */
	function woohoo_gutenberg_fse_modify_template_replace_header( $content, $template, $type, $templates ) {
		$header_type = woohoo_get_theme_option( 'header_type' );
		if ( 'custom' == $header_type ) {
			$header_style = woohoo_get_theme_option( "header_style" );
			...
			if ( strpos( $header_style, "header-fse-template-" ) !== false ) {
				...
				ob_start();
				do_action( 'woohoo_action_fse_before_custom_header', $content, $template, $type, $templates );
				...
				ob_start();
				do_action( 'woohoo_action_fse_after_custom_header', $content, $template, $type, $templates );
			...
			} else if ( woohoo_is_layouts_available() ) {
				ob_start();
				// Trigger action before the custom header to allow other modules include an additional content to the custom header
				do_action( 'woohoo_action_fse_before_custom_header', $content, $template, $type, $templates );
				// Custom header
				get_template_part( apply_filters( 'woohoo_filter_get_template_part', "templates/header-custom" ) );
				// Trigger action after the custom header to allow other modules include an additional content to the custom header
				do_action( 'woohoo_action_fse_after_custom_header', $content, $template, $type, $templates );
				// Get output
...
if ( ! function_exists( 'woohoo_gutenberg_fse_custom_header_add_side_menu' ) ) {
	add_action( 'woohoo_action_fse_after_custom_header', 'woohoo_gutenberg_fse_custom_header_add_side_menu', 10, 4 );
	/**
	 * Add a side menu to the custom header layout.
	 * 
	 * Hooks: add_action( 'woohoo_action_fse_after_custom_header', 'woohoo_gutenberg_fse_custom_header_add_side_menu', 10, 4 );
	 *
	 * @param string $content    A content of the current template.
	 * @param string $template   Path to the template. See locate_template().
	 * @param string $type       Sanitized filename without extension.
	 * @param array  $templates  A list of template candidates, in descending order of priority.
	 */
	function woohoo_gutenberg_fse_custom_header_add_side_menu( $content, $template, $type, $templates ) {
		// Side menu
...
if ( ! function_exists( 'woohoo_gutenberg_fse_custom_header_add_mobile_menu' ) ) {
	add_action( 'woohoo_action_fse_after_custom_header', 'woohoo_gutenberg_fse_custom_header_add_mobile_menu', 10, 4 );
	/**
	 * Add a mobile menu to the custom header layout.
	 * 
	 * Hooks: add_action( 'woohoo_action_fse_after_custom_header', 'woohoo_gutenberg_fse_custom_header_add_mobile_menu', 10, 4 );
	 *
	 * @param string $content    A content of the current template.
	 * @param string $template   Path to the template. See locate_template().
	 * @param string $type       Sanitized filename without extension.
	 * @param array  $templates  A list of template candidates, in descending order of priority.
	 */
	function woohoo_gutenberg_fse_custom_header_add_mobile_menu( $content, $template, $type, $templates ) {
		// Mobile menu
...
// Sidebar
//----------------------------------------------
if ( ! function_exists( 'woohoo_gutenberg_fse_add_sidebar_to_default_areas' ) ) {
	add_filter( 'default_wp_template_part_areas', 'woohoo_gutenberg_fse_add_sidebar_to_default_areas' );
	/**
	 * Filters the list of allowed template part area values and add 'sidebar' to this list.
	 * 
	 * Hook: add_filter( 'default_wp_template_part_areas', 'woohoo_gutenberg_fse_add_sidebar_to_default_areas' );
	 *
	 ...
	 */
	function woohoo_gutenberg_fse_add_sidebar_to_default_areas( $default_area_definitions ) {
		$default_area_definitions[] = array(
			'area'        => WOOHOO_FSE_TEMPLATE_PART_AREA_SIDEBAR,
			'label'       => esc_html__( 'Sidebar', 'woohoo' ),
			'description' => esc_html__( 'The Sidebar template defines a page area that typically contains a widgets.', 'woohoo' ),
			'icon'        => 'sidebar',
			'area_tag'    => 'div',
		);
...
if ( ! function_exists( 'woohoo_gutenberg_fse_modify_template_replace_sidebar' ) ) {
	add_filter( 'woohoo_filter_wp_current_template_content', 'woohoo_gutenberg_fse_modify_template_replace_sidebar', 10, 4 );
	/**
	 ...
	 * 
	 * Hooks: add_filter( 'woohoo_filter_wp_current_template_content', 'woohoo_gutenberg_fse_modify_template_replace_sidebar', 10, 4 );
	 *
	 * @param string $content    A content of the current template.
	 * @param string $template   Path to the template. See locate_template().
	 * @param string $type       Sanitized filename without extension.
	 ...
	 * @return string  A filtered content of the template.
	 */
	function woohoo_gutenberg_fse_modify_template_replace_sidebar( $content, $template, $type, $templates ) {
		...
		if ( woohoo_sidebar_present() ) {
			...
			if ( woohoo_exists_trx_addons() ) {
				...
				if ( 'custom' == $sidebar_type && ! woohoo_is_layouts_available() ) {
					$sidebar_type = 'default';
				}
				// Masks to search a sidebar block in the content
				$sidebar_start = apply_filters( 'woohoo_filter_wp_block_with_sidebar_mask_start',
												'(<!-- wp:group[\s]*{[^}]*"className":[\s]*"[^"]*sidebar[^"]*"[^>]*-->[\s]*'
												. '<div[^>]*class="[^"]*sidebar[^"]*"[^>]*>)'
												);
				$sidebar_end = apply_filters( 'woohoo_filter_wp_block_with_sidebar_mask_end',
												'(</div>[\s]*'
												. '<!-- /wp:group -->)'
												);
				...
				if ( 'default' == $sidebar_type ) {
					...
					if ( is_active_sidebar( $sidebar_name ) ) {
						...
						if ( preg_match( $sidebar_mask, $content ) ) {
							...
							$need_wrapper = apply_filters( 'woohoo_filter_wp_block_with_sidebar_wrapper_need', false );
							$content = preg_replace( '#<!-- wp:template-part[\s]*{[^}]*"slug":[\s]*"sidebar[^"]*"[^>]*/-->#U',
														( $need_wrapper
															? apply_filters(
																	'woohoo_filter_wp_block_with_sidebar_wrapper_start',
														...
														. ( $need_wrapper
															? apply_filters(
																	'woohoo_filter_wp_block_with_sidebar_wrapper_end',
																	'</div>'
																	. "\n"
																	. '<!-- /wp:group -->'
																)
				...
				// A custom sidebar (built with FSE or with any other builder)
					...
					$sidebar_style = woohoo_get_theme_option( "sidebar_style" );
					$sidebar_id = woohoo_get_custom_sidebar_id();
					...
					// Custom sidebar's layout
						...
						do_action( 'woohoo_action_fse_before_custom_sidebar', $content, $template, $type, $templates );
						// Custom sidebar
						do_action( 'woohoo_action_show_layout', $sidebar_id );
						// Trigger action after th custom sidebar to allow other modules include an additional content to the custom sidebar
						do_action( 'woohoo_action_fse_after_custom_sidebar', $content, $template, $type, $templates );
						// Get output
						$html = ob_get_contents();
						ob_end_clean();
						if ( ! empty( $html ) ) {
...
// Footer
//----------------------------------------------
if ( ! function_exists( 'woohoo_gutenberg_fse_modify_template_replace_footer' ) ) {
	add_filter( 'woohoo_filter_wp_current_template_content', 'woohoo_gutenberg_fse_modify_template_replace_footer', 10, 4 );
	/**
	 * Replace a footer in the content of the current template
	 * according to the Theme Options of the current page.
	 * 
	 * Hooks: add_filter( 'woohoo_filter_wp_current_template_content', 'woohoo_gutenberg_fse_modify_template_replace_footer', 10, 4 );
	 *
	 * @param string $content    A content of the current template.
	 * @param string $template   Path to the template. See locate_template().
	 * @param string $type       Sanitized filename without extension.
	 ...
	 * @return string  A filtered content of the template.
	 */
	function woohoo_gutenberg_fse_modify_template_replace_footer( $content, $template, $type, $templates ) {
		$footer_type = woohoo_get_theme_option( 'footer_type' );
		if ( 'custom' == $footer_type ) {
			$footer_style = woohoo_get_theme_option( "footer_style" );
			...
			if ( strpos( $footer_style, "footer-fse-template-" ) !== false ) {
				...
				ob_start();
				do_action( 'woohoo_action_fse_before_custom_footer', $content, $template, $type, $templates );
				...
				ob_start();
				do_action( 'woohoo_action_fse_after_custom_footer', $content, $template, $type, $templates );
			...
			// Custom footer's layout
				...
				do_action( 'woohoo_action_fse_before_custom_footer', $content, $template, $type, $templates );
				// Custom footer
				get_template_part( apply_filters( 'woohoo_filter_get_template_part', "templates/footer-custom" ) );
...
//------------------------------------------------------
if ( ! function_exists( 'woohoo_gutenberg_fse_replace_featured_image_renderer' ) ) {
	add_filter( 'block_type_metadata_settings', 'woohoo_gutenberg_fse_replace_featured_image_renderer', 10, 2 );
	/**
	 ...
	 */
	function woohoo_gutenberg_fse_replace_featured_image_renderer( $settings = array(), $metadata = array() ) {
		if ( ! empty( $settings['render_callback'] ) && $settings['render_callback'] == 'render_block_core_post_featured_image' ) {
			$settings['render_callback'] = 'woohoo_gutenberg_fse_featured_image_renderer';
		}
		return $settings;
	}
}

if ( ! function_exists( 'woohoo_gutenberg_fse_featured_image_renderer' ) ) {
	/**
	 ...
	 */
	function woohoo_gutenberg_fse_featured_image_renderer( $attributes, $content, $block ) {
		...
		$woohoo_hover      = woohoo_get_theme_option( 'image_hover' );
		$woohoo_components = woohoo_array_get_keys_by_value( woohoo_get_theme_option( 'meta_parts' ) );
		...
		woohoo_show_post_featured( apply_filters( 'woohoo_filter_args_featured',
			array(
				...
				'singular'   => woohoo_is_singular(),
				'thumb_size' => woohoo_get_thumb_size( strpos( woohoo_get_theme_option( 'body_style' ), 'full' ) !== false
									? 'full'
									: ( $woohoo_expanded || woohoo_is_singular()
										? 'huge' 
										: 'big' 
										)
									),
