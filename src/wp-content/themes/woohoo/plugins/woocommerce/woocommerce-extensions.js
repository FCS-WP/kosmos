...
jQuery( document ).on( 'action.ready_woohoo', function() {
		...
		if ( $product.length ) {
			...
			if ( $gallery.length ) {
				$document.on( 'action.resize_woohoo', function() {
					woohoo_woocommerce_stretch_fullwidth();
				} );
				woohoo_woocommerce_stretch_fullwidth();
	...
	function woohoo_woocommerce_stretch_fullwidth() {
	...
	if ( $body.hasClass('single-product') ) {
		...
		if ( product_wrap.length ) {
			...
			woohoo_add_filter('woohoo_filter_masonry_wrap', function( selector ) {
				selector += ( selector ? ',' : '' ) + '.woocommerce-product-gallery__wrapper';
				return selector;
			});
			// Add gallery image to the masonry item selector
			woohoo_add_filter('woohoo_filter_masonry_item', function( selector ) {
	...
	if ( WOOHOO_STORAGE['add_to_cart_sticky'] ) {

		// Show sticky bar on form is out of the window viewport
		var $form = jQuery( '.summary form.cart' );
		if ( $form.length ) {
			woohoo_intersection_observer_add( $form, function( item, enter, data ) {
				if ( ! enter ) {
		...
		if ( $footer.length ) {
			woohoo_intersection_observer_add( $footer, function( item, enter, data ) {
				if ( ! enter ) {
					if ( ! $body.hasClass( 'single_product_bottom_bar_sticky_on' ) ) {
						$body.addClass( 'single_product_bottom_bar_sticky_on' );
					}
		...
		jQuery( '.single_product_bottom_bar_button_select_options' ).on( 'click', function(e) {
			woohoo_document_animate_to( '.variations_form' );
			e.preventDefault();
			return false;
		} );
	}
	...
	jQuery( document ).on( 'action.init_hidden_elements', function( e, $cont ) {
		$cont.find( '.woohoo_woocommerce_filters_button:not(.inited),.sidebar > .sidebar_close:not(.inited),.sidebar_overlay:not(.inited)' )
			.addClass( 'inited' )
			.on( 'click', function(e) {
