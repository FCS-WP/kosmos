...
if ( ! function_exists( 'woohoo_need_page_title' ) ) {
	/**
	 * Check if the current page is need a title.
     *
     * Trigger a filter 'woohoo_filter_need_page_title' to allow other modules to modify a result.
	 *
	 * @return bool  Return true if the current page is need a title.
	 */
	function woohoo_need_page_title() {
		return ! is_front_page() && apply_filters( 'woohoo_filter_need_page_title', true );
	}
}

if ( ! function_exists( 'woohoo_show_layout' ) ) {
	/**
     ...
     */
	function woohoo_show_layout( $str, $before = '', $after = '' ) {
		if ( trim( $str ) != '' ) {
...
if ( ! function_exists( 'woohoo_get_logo_image' ) ) {
	/**
	 ...
	 */
	function woohoo_get_logo_image( $type = '' ) {
		$logo_image  = '';
		if ( empty( $type ) && function_exists( 'the_custom_logo' ) ) {
			$logo_image = woohoo_get_theme_option( 'custom_logo' );
			if ( empty( $logo_image ) ) {
			...
			$logo_image = woohoo_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) );
		}
		$logo_retina = woohoo_is_on( woohoo_get_theme_option( 'logo_retina_enabled' ) )
						? woohoo_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) . '_retina' )
						: '';
		return array(
					'logo'        => ! empty( $logo_image ) ? woohoo_remove_protocol_from_url( $logo_image, false ) : '',
					'logo_retina' => ! empty( $logo_retina ) ? woohoo_remove_protocol_from_url( $logo_retina, false ) : ''
				);
	}
}

if ( ! function_exists( 'woohoo_get_header_video' ) ) {
	/**
	 * Return a video URL to display in the header (if set).
	 *
	 * Trigger a filter 'woohoo_header_video_enable' to detect if a video allowed for the current page.
	 *
	 * @return string  URL of the video for a header
	 */
	function woohoo_get_header_video() {
		$video = '';
		if ( apply_filters( 'woohoo_header_video_enable', ! wp_is_mobile() && is_front_page() ) ) {
			if ( woohoo_check_theme_option( 'header_video' ) ) {
				$video = woohoo_get_theme_option( 'header_video' );
...
if ( ! function_exists( 'woohoo_show_post_featured_image' ) ) {
	/**
	 ...
	 */
	function woohoo_show_post_featured_image( $args = array() ) {
		...
		if ( ( ! woohoo_sc_layouts_showed( 'featured' ) && strpos( woohoo_get_post_content(), '[trx_widget_banner]' ) === false ) || in_array( $post_format, array( 'audio', 'video', 'gallery' ) ) ) {
			do_action( 'woohoo_action_before_post_featured' );
			woohoo_show_post_featured( $args );
			do_action( 'woohoo_action_after_post_featured' );
		} elseif ( woohoo_is_on( woohoo_get_theme_option( 'seo_snippets' ) ) && has_post_thumbnail() ) {
			?>
			<meta itemprop="image" itemtype="<?php echo esc_attr( woohoo_get_protocol( true ) ); ?>//schema.org/ImageObject" content="<?php echo esc_url( wp_get_attachment_url( get_post_thumbnail_id() ) ); ?>">
			<?php
		}
	}
}

if ( ! function_exists( 'woohoo_show_post_title_and_meta' ) ) {
	/**
	 ...
	 */
	function woohoo_show_post_title_and_meta( $args = array() ) {
		...
		// Title and post meta
		if ( ( ! woohoo_sc_layouts_showed( 'title' ) || ! woohoo_sc_layouts_showed( 'postmeta' ) ) ) {
			...
			<div class="post_header post_header_single entry-header">
				...
				do_action( 'woohoo_action_post_header_start' );
				
				// Post meta
				$meta_components = woohoo_array_get_keys_by_value( woohoo_get_theme_option( 'meta_parts' ) );
				$show_categories = in_array( 'categories', $meta_components );
				$meta_components = woohoo_array_delete_by_value( $meta_components, 'categories' );
				$meta_components = woohoo_array_delete_by_value( $meta_components, 'likes' );
				$share_position  = woohoo_array_get_keys_by_value( woohoo_get_theme_option( 'share_position' ) );
				if ( ! in_array( 'top', $share_position ) ) {
					$meta_components = woohoo_array_delete_by_value( $meta_components, 'share' );
				}
				$seo = woohoo_is_on( woohoo_get_theme_option( 'seo_snippets' ) );
				if ( ! empty( $args['show_title'] ) ) {
					if ( $show_categories && ! woohoo_sc_layouts_showed( 'postmeta' ) && woohoo_is_on( woohoo_get_theme_option( 'show_post_meta' ) ) ) {
					...
					// Post title
					if ( ! woohoo_sc_layouts_showed( 'title' ) ) {
						the_title( '<h1 class="post_title entry-title"' . ( $seo ? ' itemprop="headline"' : '' ) . '>', '</h1>' );
					}
					// Post subtitle
					$post_subtitle = woohoo_get_theme_option( 'post_subtitle' );
					if ( ! empty( $post_subtitle ) ) {
						?>
						<div class="post_subtitle">
							<?php woohoo_show_layout( $post_subtitle ); ?>
						</div>
						<?php
					}
				}
				// Post meta
				if ( ! empty( $args['show_meta'] ) && ! woohoo_sc_layouts_showed( 'postmeta' ) && woohoo_is_on( woohoo_get_theme_option( 'show_post_meta' ) ) ) {
					if ( ! empty( $args['split_meta_by'] ) ) {
						?><div class="post_meta_other"><?php
							$meta_components = woohoo_get_theme_option( 'meta_parts' );
							...
							$part1 = array_keys( woohoo_array_slice( $meta_components, '', $args['split_meta_by'] ), 1 );
							$part2 = array_keys( woohoo_array_slice( $meta_components, '+' . $args['split_meta_by'], '' ), 1 );
							if ( ! in_array( 'top', $share_position ) ) {
								$part2 = woohoo_array_delete_by_value( $part2, 'share' );
							}
							woohoo_show_post_meta(
								apply_filters(
									'woohoo_filter_post_meta_args',
							...
							woohoo_show_post_meta(
								apply_filters(
									...
									array_merge(
										...
										$args,
										array(
											'show_labels' => ! empty( $args['show_labels'] ) || ! in_array( 'share', $part2 ) || ! woohoo_exists_trx_addons()
										)
									),
									'single',
									1
				...
				do_action( 'woohoo_action_post_header_end' );
			...
			ob_end_clean();
			if ( strpos( $woohoo_post_header, 'post_subtitle' ) !== false
...
if ( ! function_exists( 'woohoo_show_post_content' ) ) {
	/**
	 ...
	 */
	function woohoo_show_post_content( $args = array(), $otag='', $ctag='' ) {
		$simple = true;
		...
		if ( has_excerpt() ) {
			...
			woohoo_show_layout( woohoo_filter_post_content( get_the_content('') ) );
			...
			$quote = woohoo_get_tag( woohoo_filter_post_content( get_the_content() ), '<blockquote', '</blockquote>' );
			if ( ! empty( $quote ) ) {
				woohoo_show_layout( wpautop( $quote ) );
				$simple = false;
			} else {
				woohoo_show_layout( woohoo_filter_post_content( get_the_content() ) );
			}
			...
			if ( $simple ) {
				$len = isset( $args['hide_excerpt'] ) && (int) $args['hide_excerpt'] > 0
							...
							: ( ! empty( $args['excerpt_length'] )
								...
								: woohoo_get_theme_option( 'excerpt_length' )
								);
				$output = woohoo_excerpt( $output, $len );
			}
		}
		if ( trim( str_replace( "&nbsp;", '', $output ) ) != '' ) {
			woohoo_show_layout( trim( $output ), $otag, $ctag);
		}
	}
}
...
if ( ! function_exists( 'woohoo_show_post_comments_link' ) ) {
	/**
	 ...
	 * @param string $ctag  Optional. A close tag to wrap a result. Default is empty string.
	 */
	function woohoo_show_post_comments_link( $args = array(), $otag='', $ctag='' ) {
		...
		woohoo_show_layout(
			'<a class="more-link comments-link" href="' . esc_url( get_comments_link() ) . '">'
				. ( ! empty( $args['comments_text'] )
						...
						: ( $total == 0
							? esc_html__( 'Leave a comment', 'woohoo' )
							: ( $total == 1 ? esc_html__( 'View comment', 'woohoo' ) : esc_html__( 'View comments', 'woohoo' ) )
							)
						)
			. '</a>',
			$otag,
...
if ( ! function_exists( 'woohoo_show_post_pagination' ) ) {
	/**
	 * Show the single post pagination links.
	 *
	 * Trigger actions: 'woohoo_action_before_post_pagination' and 'woohoo_action_after_post_pagination'
	 * to allow other modules wrap the result.
	 */
	function woohoo_show_post_pagination() {
		do_action( 'woohoo_action_before_post_pagination' );
		wp_link_pages(
			array(
		...
		do_action( 'woohoo_action_after_post_pagination' );
	}
}

if ( ! function_exists( 'woohoo_show_post_footer' ) ) {
	/**
     ...
     */
	function woohoo_show_post_footer( $components = 'pages,tags,likes,share,prev_next,author' ) {
		...
		foreach( $components as $comp ) {
			...
			if ( 'tags' == $comp ) {
				...
				ob_start();
				the_tags( '', '', '' );
				$woohoo_meta_output = ob_get_contents();
				ob_end_clean();
				if ( ! empty( $woohoo_meta_output ) ) {
					do_action( 'woohoo_action_before_post_tags' );
					woohoo_show_layout( $woohoo_meta_output, '<div class="post_tags_single"><span class="post_meta_label">' . esc_html__( 'Tags:', 'woohoo' ) . '</span> ', '</div>' );
					do_action( 'woohoo_action_after_post_tags' );
				}

			} else if ( 'likes' == $comp ) {

				// Emotions
				if ( woohoo_exists_trx_addons() && function_exists( 'trx_addons_get_post_reactions' ) && trx_addons_is_on( trx_addons_get_option( 'emotions_allowed' ) ) ) {
				...
				if ( in_array( 'likes', $components ) && in_array( 'likes', $meta_components )
						&&
						( ! function_exists( 'trx_addons_get_option' ) || trx_addons_is_off( trx_addons_get_option( 'emotions_allowed' ) ) || ! apply_filters( 'trx_addons_filter_show_post_reactions', woohoo_is_single() && ! is_attachment() ) )
				) {
					$meta_footer[] = 'likes';
				}
				...
				if ( count( $meta_footer) > 0 ) {
					...
					if ( ! empty( $woohoo_meta_output ) ) {
						...
						woohoo_show_layout( $woohoo_meta_output );
				...
				// Author bio
				if ( woohoo_get_theme_option( 'show_author_info' ) == 1
					&& ! is_attachment()
					&& get_the_author_meta( 'description' )
					&& ( 'scroll' != $woohoo_posts_navigation || woohoo_get_theme_option( 'posts_navigation_scroll_hide_author' ) == 0 )
					&& ( ! $full_post_loading || woohoo_get_theme_option( 'open_full_post_hide_author' ) == 0 )
				) {
					do_action( 'woohoo_action_before_post_author' );
					get_template_part( apply_filters( 'woohoo_filter_get_template_part', 'templates/author-bio' ) );
					do_action( 'woohoo_action_after_post_author' );
				}

			} else if ( 'prev_next' == $comp ) {

				// Previous/next post navigation.
				if ( 'links' == $woohoo_posts_navigation && ! $full_post_loading ) {
					do_action( 'woohoo_action_before_post_navigation' );
					?>
					<div class="nav-links-single<?php
						if ( woohoo_get_theme_setting( 'thumbs_in_navigation' ) ) {
					...
					?>">
						...
						the_post_navigation( apply_filters( 'woohoo_filter_post_navigation_args', array(
							'next_text' => ( woohoo_get_theme_setting( 'thumbs_in_navigation' ) ? '<span class="nav-arrow"></span>' : '' )
								. '<span class="nav-arrow-label">' . esc_html__( 'Next', 'woohoo' ) . '</span> '
								. '<h6 class="post-title">%title</h6>'
								. '<span class="post_date">%date</span>',
							'prev_text' => ( woohoo_get_theme_setting( 'thumbs_in_navigation' ) ? '<span class="nav-arrow"></span>' : '' )
								. '<span class="nav-arrow-label">' . esc_html__( 'Previous', 'woohoo' ) . '</span> '
								. '<h6 class="post-title">%title</h6>'
								. '<span class="post_date">%date</span>',
						), 'post_footer' ) );
						?>
					...
					<?php
					do_action( 'woohoo_action_after_post_navigation' );
				}
...
if ( ! function_exists( 'woohoo_show_post_meta' ) ) {
	/**
	 ...
	 */
	function woohoo_show_post_meta( $args = array() ) {
		if ( woohoo_is_single() && woohoo_is_off( woohoo_get_theme_option( 'show_post_meta' ) ) ) {
			return ' ';  // Need at least one space!
		}
		$args = array_merge(
			apply_filters( 'woohoo_filter_post_meta_args_default', array(
		...
		if ( woohoo_storage_isset( 'options', 'meta_parts', 'val' ) ) {
			$parts = explode( '|', woohoo_get_theme_option( 'meta_parts' ) );
			$list_new = array();
			foreach( $parts as $part ) {
				$part = explode( '=', $part );
				if ( in_array( $part[0], $components ) ) {
					$list_new[] = $part[0];
					$components = woohoo_array_delete_by_value( $components, $part[0] );
				}
			}
		...
		foreach ( $components as $comp ) {
			...
			if ( 'categories' == $comp ) {
					...
					if ( ! empty( $meta['sponsored_post'] ) && 1 == (int) $meta['sponsored_post'] ) {
						$cats = ( ! empty( $meta['sponsored_url'] )
								...
								. ( ! empty( $meta['sponsored_label'] )
									...
									: esc_html__( 'Sponsored content', 'woohoo' )
									)
								. ( ! empty( $meta['sponsored_url'] )
									? '</a>'
									: '</span>'
									);
						woohoo_show_layout( $cats, '<span class="post_meta_item post_sponsored">', '</span>');
					}
				}
				// Post categories
				$cats = get_post_type() == 'post'
								...
								: apply_filters( 'woohoo_filter_get_post_categories', '' );
				if ( ! empty( $cats ) ) {
					woohoo_show_layout( $cats, 
				...
				// Published date
				$dt = apply_filters( 'woohoo_filter_get_post_date', woohoo_get_date( '', ! empty( $args['date_format'] ) ? $args['date_format'] : '' ) );
				if ( ! empty( $dt ) && ( empty( $dt_last ) || $dt_last != $dt ) ) {
					woohoo_show_layout(
						...
						'<span class="post_meta_item post_date' . ( ! empty( $args['seo'] ) ? ' date published' : '' ) . '"'
							...
							. '>'
							. ( ! woohoo_is_single() ? '<a href="' . esc_url( get_permalink() ) . '">' : '' )
							. ( in_array( 'date', $components ) && in_array( 'modified', $components ) && get_post_type() == 'post' ? '<span class="post_meta_item_label">' . esc_html__( 'Published:', 'woohoo' ) . '</span>' : '' ),
						( ! woohoo_is_single() ? '</a>' : '' ) . '</span>'
					);
					$dt_last = $dt;
				}
			} elseif ( 'modified' == $comp && get_post_type() == 'post' ) {
				// Modified date
				$dt = apply_filters( 'woohoo_filter_get_post_modified_date', woohoo_get_date( get_post_modified_time( 'U' ), ! empty( $args['date_format'] ) ? $args['date_format'] : '' ) );
				if ( ! empty( $dt ) && ( empty( $dt_last ) || $dt_last != $dt ) ) {
					woohoo_show_layout(
						$dt,
						'<span class="post_meta_item post_date' . ( ! empty( $args['seo'] ) ? ' date updated modified' : '' ) . '"'
							...
							. '>'
							. ( ! woohoo_is_single() ? '<a href="' . esc_url( get_permalink() ) . '">' : '' )
							. '<span class="post_meta_item_label">' . esc_html__( 'Updated:', 'woohoo' ) . '</span>',
						( ! woohoo_is_single() ? '</a>' : '' ) . '</span>'
					);
					$dt_last = $dt;
				}
				...
				if ( $author_id > 0 ) {
					...
					$author_avatar = ! empty( $args['author_avatar'] )
										? get_avatar( get_the_author_meta( 'user_email', $author_id ), apply_filters( 'woohoo_filter_author_avatar_size', 56, 'post_meta' ) * woohoo_get_retina_multiplier() ) 
										: '';
					echo '<a class="post_meta_item post_author" rel="author" href="' . esc_url( $author_link ) . '">'
							. apply_filters( 'woohoo_filter_post_author_content',
								'<span class="post_author_by">' . esc_html__( 'By', 'woohoo' ) . '</span>'
								. ( ! empty( $author_avatar )
				...
				// Comments
				if ( !woohoo_is_single() || have_comments() || comments_open() ) {
					$post_comments = get_comments_number();
					echo '<a href="' . esc_url( get_comments_link() ) . '" class="post_meta_item post_meta_comments icon-comment-light">'
							. '<span class="post_meta_number">' . esc_html( woohoo_num2size( $post_comments ) ) . '</span>'
							. ( $args['show_labels'] ? '<span class="post_meta_label">' . esc_html( _n( 'Comment', 'Comments', $post_comments, 'woohoo' ) ) . '</span>' : '' )
						. '</a>';
				}
			...
			// Views
				...
				if ( function_exists( 'trx_addons_get_post_views' ) ) {
					...
					echo '<a href="' . esc_url( get_permalink() ) . '" class="post_meta_item post_meta_views trx_addons_icon-eye">'
							. '<span class="post_meta_number">' . esc_html( woohoo_num2size( $post_views ) ) . '</span>'
							. ( $args['show_labels'] ? '<span class="post_meta_label">' . esc_html( _n( 'View', 'Views', $post_views, 'woohoo' ) ) . '</span>' : '' )
						. '</a>';
				}
			...
			// Likes (Emotions)
				...
				if ( function_exists( 'trx_addons_get_post_likes' ) ) {
					...
					$liked = woohoo_get_cookie( 'trx_addons_likes' );
					...
					echo ( true == $emotions_allowed
							...
							: '<a href="#"'
								...
								. ' title="' . ( ! empty( $allow ) ? esc_attr__( 'Like', 'woohoo') : esc_attr__( 'Dislike', 'woohoo' ) ) . '"'
								. ' data-postid="' . esc_attr( get_the_ID() ) . '"'
								. ' data-likes="' . esc_attr( $post_likes ) . '"'
								. ' data-title-like="' . esc_attr__( 'Like', 'woohoo') . '"'
								. ' data-title-dislike="' . esc_attr__( 'Dislike', 'woohoo' ) . '"'
								. '>'
							)
								. '<span class="post_meta_number">' . esc_html( woohoo_num2size( $post_likes ) )  . '</span>'
								. ( $args['show_labels']
									? '<span class="post_meta_label">'
										. ( true == $emotions_allowed
											? esc_html( _n( 'Reaction', 'Reactions', $post_likes, 'woohoo' ) )
											: esc_html( _n( 'Like', 'Likes', $post_likes, 'woohoo' ) )
											)
									. '</span>'
									: '' )
							. '</a>';
				...
				// Socials share
				woohoo_show_share_links( apply_filters( 'woohoo_filter_share_links_args', 
					array(
						'type'      => $args['share_type'],
						'wrap'      => 'span',
						'direction' => $args['share_direction'],
						'caption'   => 'drop' == $args['share_type']
											? apply_filters( 'woohoo_filter_share_links_text', esc_html__( 'Share', 'woohoo' ) )
											: '',
				...
				// Edit page link
				edit_post_link( esc_html__( 'Edit', 'woohoo' ), '', '', 0, 'post_meta_item post_edit icon-pencil' );

			} else {
				// Custom counter
				do_action( 'woohoo_action_show_post_meta', $comp, get_the_ID(), $args );
			}
...
if ( ! function_exists( 'woohoo_show_post_featured' ) ) {
	/**
	 * Show a post featured block: image, video, audio, etc.
	 *
	 * @param array $args  Optional. Additional options to build a block layout.
	 */
	function woohoo_show_post_featured( $args = array() ) {
		$args = apply_filters( 'woohoo_filter_post_featured_args', array_merge(
			array(
				'popup'         => ! woohoo_is_single() && woohoo_get_theme_option( 'video_in_popup' ),  // Open video in popup
				'hover'         => woohoo_get_theme_option( 'image_hover' ),     // Hover effect
				'parallax'      => woohoo_get_theme_option( 'single_parallax' ), // Parallax speed. If 0 - no parallax effect is used
				...
				'show_no_image' => woohoo_is_on( woohoo_get_theme_setting( 'allow_no_image' ) ),  // Display 'no-image.jpg' if post haven't thumbnail
				'video'         => '',                                            // Video layout
				'autoplay'      => false,                                         // Autoplay video (if present)
				'seo'           => woohoo_is_on( woohoo_get_theme_option( 'seo_snippets' ) ),     // Add SEO-snippets
		...
		$thumb_size        = ! empty( $args['thumb_size'] )
								? $args['thumb_size']
								: woohoo_get_thumb_size( is_attachment() || woohoo_is_single() ? 'full' : 'big' );
		$has_thumb         = has_post_thumbnail();
		$thumb_id          = 0;

		$parallax          = ! empty( $args['parallax'] ) && woohoo_is_singular('post') && ! in_array( $post_format, array( 'audio', 'video', 'gallery' ) )
								? $args['parallax']
								: 0;

		if ( 'audio' == $post_format ) {
			$audios = woohoo_get_post_audio_list();
			if ( ! empty( $audios[0]['cover'] ) ) {
				$has_thumb = true;
				$thumb_id  = woohoo_attachment_url_to_postid( $audios[0]['cover'] );
				if ( empty( $args['thumb_size'] ) ) {
					$ts = woohoo_detect_thumb_size( $audios[0]['cover'] );
					if ( ! empty( $ts ) ) {
						$thumb_size = $ts;
					}
				}
			...
			$videos = woohoo_get_post_video_list();
			if ( $post_meta === false ) {
			...
			if ( ( ! empty( $args['singular'] ) && ! empty( $post_meta['video_without_cover'] ) ) || ! empty( $args['autoplay'] ) ) {
				...
				$has_thumb = true;
				$thumb_id  = woohoo_attachment_url_to_postid( $videos[0]['image'] );
				if ( empty( $args['thumb_size'] ) ) {
					$ts = woohoo_detect_thumb_size( $videos[0]['image'] );
					if ( ! empty( $ts ) ) {
						$thumb_size = $ts;
					}
				}				
		...
		$no_image = ! empty( $args['show_no_image'] ) ? woohoo_get_no_image( '', true ) : '';
		...
		if ( $args['thumb_bg'] ) {
			if ( $has_thumb ) {
				...
				$image = woohoo_get_post_image();
				if ( ! empty( $image ) ) {
					$image = woohoo_add_thumb_size( $image, $thumb_size );
				}
			}
			if ( empty( $image ) ) {
				$image = $no_image;
			...
			if ( ! empty( $image ) ) {
				...
				$args['thumb_bg_image'] = woohoo_add_inline_css_class( 'background-image: url(' . esc_url( $image ) . ');' );
			} else {
				$args['thumb_bg'] = false;
			}
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured post_attachment
						...
						echo ( ! empty( $args['class'] ) ? ' ' . esc_attr( $args['class'] ) : '' )
							. ( ! empty( $args['css'] ) ? ' ' . woohoo_add_inline_css_class( $args['css'] ) : '' )
							. ( ! empty( $args['thumb_bg_class'] ) ? ' ' . esc_attr( $args['thumb_bg_class'] ) . ' ' . esc_attr( $args['thumb_bg_image'] ) : '' );
						?>
					">
				<?php
				do_action('woohoo_action_before_featured');
				if ( ! $args['thumb_bg'] ) {
					echo wp_get_attachment_image(
							get_the_ID(),
							$thumb_size,
							false,
							woohoo_is_on( woohoo_get_theme_option( 'seo_snippets' ) )
								? array( 'itemprop' => 'image' )
								: ''
					);
				}
				if ( woohoo_get_theme_setting( 'attachments_navigation' ) ) {
					?>
				...
				do_action('woohoo_action_after_featured');
				?>
				</div>
				<?php
				if ( has_excerpt() ) {
				...
				if ( ( ( $has_thumb || ! empty( $args['show_no_image'] ) ) && ! woohoo_sc_layouts_showed( 'featured' ) )
						|| $show_video_player || ! empty( $videos )
						|| $show_audio_player
						|| $show_gallery
				) {
					$output = '<div class="'
									. apply_filters( 'woohoo_filter_post_featured_classes',
										'post_featured'
					...
					if ( $args['seo'] ) {
						$output .= ' itemscope="itemscope" itemprop="image" itemtype="' . esc_attr( woohoo_get_protocol( true ) ) . '//schema.org/ImageObject"';
					}
					...
					$args['data'] = apply_filters( 'woohoo_filter_post_featured_data', $args['data'], $args, 'singular' );
					if ( ! empty( $args['data'] ) && is_array( $args['data'] ) ) {
					...
					if ( $has_thumb && $args['seo'] ) {
						$woohoo_attr = woohoo_getimagesize( wp_get_attachment_url( $thumb_id ) );
						?>
						<meta itemprop="width" content="<?php echo esc_attr( $woohoo_attr[0] ); ?>">
						<meta itemprop="height" content="<?php echo esc_attr( $woohoo_attr[1] ); ?>">
						<?php
					}
					...
					woohoo_show_post_avg( array_merge( $args, compact( 'post_format', 'has_thumb', 'thumb_id', 'thumb_size', 'videos', 'audios', 'gallery_images', 'post_meta' ) ) );
					// Put optional info block over the thumb
					woohoo_show_layout( $args['post_info'] );
			...
			if ( $has_thumb
				...
				|| ( ! $args['thumb_only']
						&& ( in_array( $post_format, array( 'image', 'audio', 'video' ) )
							|| ( 'gallery' == $post_format && woohoo_exists_trx_addons() )
							)
					)
			) {
				$output = '<div class="'
				...
				$args['data'] = apply_filters( 'woohoo_filter_post_featured_data', $args['data'], $args, 'blog' );
				if ( ! empty( $args['data'] ) && is_array( $args['data'] ) ) {
					foreach( $args['data'] as $k => $v ) {
						$output .= ' data-' . esc_attr( $k ) . '="' . esc_attr( $v ) . '"';
					}
				...
				// Show hover
				if ( $show_hover && function_exists( 'woohoo_hovers_add_icons' ) ) {
					if ( ! empty( $args['hover'] ) ) {
						?>
						<div class="mask"></div>
						<?php
					}
					woohoo_hovers_add_icons(
						$args['hover'],
						array(
				...
				// Add audio, video or gallery
				woohoo_show_post_avg( array_merge( $args, compact( 'post_format', 'has_thumb', 'thumb_id', 'thumb_size', 'videos', 'audios' ) ) );
			...
			if ( 'video' == $args['post_format'] || ! empty( $args['video'] ) ) {
				if ( ! empty( $args['singular'] ) 
					...
					<div class="post_video_list">
						<?php
						woohoo_show_layout( trx_addons_sc_widget_video_list( array(
					...
					if ( ! empty( $args['video'] ) ) {
						...
						$video = woohoo_get_post_video_list_first( true );
					} else if ( woohoo_get_theme_option( 'blog_content' ) != 'fullpost' ) {
						$video = woohoo_get_post_video( $post_content, false );
					}
					if ( empty( $args['singular'] ) ) {
						if ( empty( $video ) ) {
							$video = woohoo_get_post_iframe( $post_content, false );
						}
						if ( empty( $video ) ) {
							// Only get video from the content if a playlist isn't present.
							$post_content_parsed = woohoo_filter_post_content( $post_content );
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $videos ) && is_array( $videos ) ) {
									$video = woohoo_array_get_first( $videos, false );
								}
							}
						}
					}
						...
						if ( $args['has_thumb'] && ! empty( $args['popup'] ) && function_exists( 'trx_addons_get_video_layout' ) ) {
							...
							if ( ! empty( $popup[0] ) && ! empty( $popup[1] ) ) {
								if ( preg_match( '/<a .*<\/a>/', $popup[0], $matches ) && ! empty( $matches[0] ) ) {
									$video_out = true;
									?>
									<div class="post_video_hover post_video_hover_popup"><?php woohoo_show_layout( $matches[0] ); ?></div>
									<?php
									woohoo_show_layout($popup[1]);
								}
							}
						}
						if ( ! $video_out ) {
							if ( ! empty( $args['autoplay'] ) ) {
								$video = woohoo_make_video_autoplay( $video, true );
							} else if ( ! empty( $args['has_thumb'] ) ) {
								$video = woohoo_make_video_autoplay( $video );
								?>
								<div class="post_video_hover" data-video="<?php echo esc_attr( $video ); ?>"></div>
								<?php
							}
							do_action( 'woohoo_action_before_single_post_video', $args );
							?>
							...
							?>">
								...
								if ( empty( $args['has_thumb'] ) || ! empty( $args['autoplay'] ) ) {
									woohoo_show_layout( $video );
								}
								?>
							</div>
							<?php
							do_action( 'woohoo_action_after_single_post_video', $args );
						}
					}
				}
				...
				if ( ! empty( $args['audios'][0] ) ) {
					...
					$audio             = woohoo_get_post_audio_list_first( true );
				}
				if ( empty( $args['singular'] ) ) {
					if ( empty( $audio ) && woohoo_get_theme_option( 'blog_content' ) != 'fullpost' ) {
						$audio = woohoo_get_post_audio( $post_content, false );
						if ( empty( $audio ) ) {
							$audio = woohoo_get_post_iframe( $post_content, false );
						}
					}
					...
					if ( empty( $audio ) && woohoo_get_theme_option( 'blog_content' ) != 'fullpost' ) {
						...
						if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
							...
							if ( ! empty( $audios ) && is_array( $audios ) ) {
								$audio = woohoo_array_get_first( $audios, false );
							}
						}
					}
				}
					...
					">
						...
						if ( empty( $args['singular'] ) && empty( $media_author ) && empty( $media_title ) ) {
							if ( strpos( $audio, '<audio' ) !== false ) {
								$media_author = woohoo_get_tag_attrib( $audio, '<audio>', 'data-author' );
								$media_title  = woohoo_get_tag_attrib( $audio, '<audio>', 'data-caption' );
							}
							if ( empty( $media_author) &&  empty( $media_title) ) {
								$media = urldecode( woohoo_get_tag_attrib( $post_content, '[trx_widget_audio]', 'media' ) );
								if ( ! empty( $media ) ) {
									...
									// Parse tag params
									$media_author = strip_tags( woohoo_get_tag( $post_content_parsed, '<h6 class="audio_author">', '</h6>' ) );
									$media_title  = strip_tags( woohoo_get_tag( $post_content_parsed, '<h5 class="audio_caption">', '</h5>' ) );
						...
						if ( ! empty( $media_author) || ! empty( $media_title) ) {
							...
							<div class="post_info_audio">
								...
								if ( ! empty( $media_author ) ) {
									woohoo_show_layout( $media_author, '<div class="post_audio_author">', '</div>' );
								}
								if ( ! empty( $media_title ) ) {
									woohoo_show_layout( $media_title, '<h5 class="post_audio_title">', '</h5>' );
								}
								if ( ! empty( $media_description ) ) {
									woohoo_show_layout( $media_description, '<div class="post_audio_description">', '</div>' );
								}
								?>
							</div>
							<?php
						...
						// Display audio
						woohoo_show_layout( $audio, '<div class="audio_wrap">', '</div>' );
						?>
					</div>
					<?php
				}
				...
				$slider_args['slides_ratio'] = ! empty( $args['thumb_ratio'] ) && ! in_array( $args['thumb_ratio'], array('none', 'masonry') )
													? $args['thumb_ratio']
													: '16:9';
				$output = woohoo_get_slider_layout( apply_filters( 'woohoo_filter_post_slider_args', $slider_args ) );
				if ( '' != $output ) {
					woohoo_show_layout( $output );
				}
			}
...
if ( ! function_exists( 'woohoo_is_with_featured_image' ) ) {
	/**
	 ...
	 */
	function woohoo_is_with_featured_image( $content, $disabled=array() ) {
...
if ( ! function_exists( 'woohoo_get_no_image' ) ) {
	/**
	 ...
	 */
	function woohoo_get_no_image( $no_image = '', $need = false ) {
		static $no_image_url = '';
		$img = woohoo_get_theme_option( 'no_image' );
		if ( empty( $img ) && ( $need || woohoo_get_theme_setting( 'allow_no_image' ) ) ) {
			if ( empty( $no_image_url ) ) {
				$no_image_url = woohoo_get_file_url( 'images/no-image.jpg' );
			}
			$img = $no_image_url;
		}
		if ( ! empty( $img ) ) {
...
if ( ! function_exists( 'woohoo_add_bg_in_post_nav' ) ) {
	/**
	 ...
	 */
	function woohoo_add_bg_in_post_nav() {
		if ( ! woohoo_is_single() || ! woohoo_get_theme_setting( 'thumbs_in_navigation' ) ) {
			return;
		}
		...
		if ( $previous ) {
			...
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), woohoo_get_thumb_size( 'med' ) );
			...
			if ( ! empty( $img ) ) {
				...
				$css .= apply_filters( 'woohoo_filter_post_navigation_previous_css',
		...
		if ( $next ) {
			...
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), woohoo_get_thumb_size( 'med' ) );
				$img = empty( $img[0] ) ? '' : $img[0];
			} else {
				$img = $noimg;
			}
			if ( ! empty( $img ) ) {
				...
				$css .= apply_filters( 'woohoo_filter_post_navigation_next_css',
		...
		woohoo_add_inline_css( $css );
	}
}

if ( ! function_exists( 'woohoo_show_related_posts_callback' ) ) {
	add_action( 'woohoo_action_related_posts', 'woohoo_show_related_posts_callback' );
	/**
	 * Callback for action 'Related posts' to display a related posts in the different places of the theme.
	 *
	 * Hooks: add_action( 'woohoo_action_related_posts', 'woohoo_show_related_posts_callback' );
	 */
	function woohoo_show_related_posts_callback() {
		if ( woohoo_is_single() && ! apply_filters( 'woohoo_filter_show_related_posts', false ) ) {
			$woohoo_related_posts   = (int) woohoo_get_theme_option( 'related_posts' );
			$woohoo_related_columns = (int) woohoo_get_theme_option( 'related_columns' );
			$woohoo_related_style   = woohoo_get_theme_option( 'related_style' );
			if ( (int) woohoo_get_theme_option( 'show_related_posts' ) && $woohoo_related_posts > 0 ) {
				woohoo_show_related_posts(
					array(
						'orderby'        => 'rand',
						'posts_per_page' => max( 1, min( 9, $woohoo_related_posts ) ),
						'columns'        => max( 1, min( 6, $woohoo_related_posts, $woohoo_related_columns ) ),
					),
					$woohoo_related_style
				);
			}
		}
	}
...
if ( ! function_exists( 'woohoo_show_related_posts' ) ) {
	/**
	 ...
	 */
	function woohoo_show_related_posts( $args = array(), $style = 1, $title = '' ) {
		$args = array_merge(
			array(
		...
		$taxonomy = 'post' == $args['post_type'] ? 'category' : woohoo_get_post_type_taxonomy();
		...
		if ( $query->found_posts > 0 ) {
			...
			$args['slider'] = (int) woohoo_get_theme_option( 'related_slider' ) && min( $args['posts_per_page'], $query->found_posts) > $columns;
			$related_position = woohoo_get_theme_option( 'related_position' );
			$related_style = woohoo_get_theme_option( 'related_style' );
			$related_tag = strpos( $related_position, 'inside' ) === 0 ? 'h5' : 'h3';
			if ( in_array( $related_position, array( 'inside_left', 'inside_right' ) ) ) {
				$columns = 1;
			}

			do_action( 'woohoo_action_before_related_wrap' );
			?>
			<section class="related_wrap related_position_<?php echo esc_attr( $related_position ); ?> related_style_<?php echo esc_attr( $related_style ); ?>">
				...
				<?php do_action( 'woohoo_action_before_related_wrap_title' ); ?>

				<<?php echo esc_html( $related_tag ); ?> class="section_title related_wrap_title"><?php
					if ( ! empty( $title ) ) {
						echo esc_html( $title );
					} else {
						esc_html_e( 'You May Also Like', 'woohoo' );
					}
				?></<?php echo esc_html( $related_tag ); ?>><?php

				do_action( 'woohoo_action_after_related_wrap_title' );

				if ( $args['slider'] ) {
					$slider_args                      = $args;
					$slider_args['count']             = max(1, $query->found_posts);
					$slider_args['slides_min_width']  = 250;
					$slider_args['slides_space']      = woohoo_get_theme_option( 'related_slider_space' );
					$slider_args['slider_controls']   = woohoo_get_theme_option( 'related_slider_controls' );
					$slider_args['slider_pagination'] = woohoo_get_theme_option( 'related_slider_pagination' );
					$slider_args                      = apply_filters( 'woohoo_related_posts_slider_args', $slider_args, $query );
					?><div class="related_wrap_slider"><?php
					woohoo_get_slider_wrap_start('related_posts_wrap', $slider_args);
				} else {
					...
					while ( $query->have_posts() ) {
						...
						if ( ! apply_filters( 'woohoo_filter_related_post_showed', false, $args, $style ) ) {
							get_template_part( apply_filters( 'woohoo_filter_get_template_part', 'templates/related-posts', $style ), $style );
						}
						?></div><?php
					}
				?></div><?php
				if ( $args['slider'] ) {
					woohoo_get_slider_wrap_end('related_posts_wrap', $slider_args);
					?></div><?php
				}
				wp_reset_postdata();
				?>
			...
			<?php
			do_action( 'woohoo_action_after_related_wrap' );
		}
	}
}

if ( ! function_exists( 'woohoo_is_blog_style_use_masonry' ) ) {
	/**
	 ...
	 */
	function woohoo_is_blog_style_use_masonry( $style ) {
		$blog_styles = woohoo_storage_get( 'blog_styles' );
		return ! empty( $blog_styles[ $style ][ 'scripts' ] ) && in_array( 'masonry', (array) $blog_styles[ $style ][ 'scripts'] );
	}
}

if ( ! function_exists( 'woohoo_show_filters' ) ) {
	/**
	 ...
	 *                                ]
	 */
	function woohoo_show_filters( $args = array() ) {
		$args = array_merge(
			array(
				'post_type'  => '',
				'taxonomy'   => '',
		...
		if ( is_array( $terms ) && count( $terms ) > 0 ) {
			$tabs[ $args['parent_cat'] ] = esc_html__( 'All', 'woohoo' );
		...
		if ( count( $tabs ) > 0 ) {
			$filters_ajax   = woohoo_get_theme_setting( 'blog_filters_use_ajax' );
			$filters_active = $args['parent_cat'];
			$filters_id     = 'woohoo_filters';
			?>
			<div class="woohoo_tabs woohoo_tabs_ajax woohoo_filters">
				<ul class="woohoo_tabs_titles">
					<?php
					foreach ( $tabs as $tab_id => $tab_title ) {
						?><li>
							<a href="<?php echo esc_url( woohoo_get_hash_link( sprintf( '#%1$s_%2$s_content', $filters_id, $tab_id ) ) ); ?>"
				...
				foreach ( $tabs as $tab_id => $tab_title ) {
					...
					<div id="<?php echo esc_attr( sprintf( '%1$s_%2$s_content', $filters_id, $tab_id ) ); ?>"
						class="woohoo_tabs_content"
						data-blog-template="<?php echo esc_attr( woohoo_storage_get( 'blog_template' ) ); ?>"
					...
					>
						<?php
						if ( $tab_need_content ) {
							woohoo_show_posts( array_merge( $args, array( 'cat' => $tab_id ) ) );
						}
						?>
					</div>
					<?php
			...
			woohoo_show_posts( array_merge( $args, array( 'cat' => $args['parent_cat'] ) ) );
		}
	}
}

if ( ! function_exists( 'woohoo_show_posts' ) ) {
	/**
	 ...
	 */
	function woohoo_show_posts( $args = array() ) {
		$args = array_merge(
			array(
				'post_type'      => 'post',
				'taxonomy'       => 'category',
				'parent_cat'     => 0,
				'cat'            => 0,
				'posts_per_page' => (int) woohoo_get_theme_option( 'posts_per_page' ),
				'page'           => 1,
				'sticky'         => false,
				'blog_style'     => '',
				'echo'           => true,
		...
		$blog_style   = empty( $args['blog_style'] ) ? woohoo_get_theme_option( 'blog_style' ) : $args['blog_style'];
		...
		if ( strpos( $style, 'blog-custom-' ) === 0 ) {
			$custom_blog_id   = woohoo_get_custom_blog_id( $style );
			$custom_blog_meta = woohoo_get_custom_layout_meta( $custom_blog_id );
			if ( ! empty( $custom_blog_meta['margin'] ) ) {
				woohoo_add_inline_css( sprintf( '.page_content_wrap{padding-top:%s}', esc_attr( woohoo_prepare_css_value( $custom_blog_meta['margin'] ) ) ) );
			}
		...
		if ( ! $args['echo'] ) {
			...
			$q_args = woohoo_query_add_posts_and_cats( $q_args, '', $args['post_type'], $args['cat'], $args['taxonomy'] );
			if ( $args['page'] > 1 ) {
		...
		if ( woohoo_is_blog_style_use_masonry( $style ) ) {
			woohoo_lazy_load_off();
		}

		// Show posts
		$class = apply_filters( 'woohoo_filter_posts_container_classes', 'posts_container'
				. sprintf( ' %1$s_wrap %1$s_%2$d', $style, $columns )
				. ( ! woohoo_is_off( $custom_style )
					? sprintf( ' %s_wrap', $custom_style ) . ( 'masonry' == $custom_style ? sprintf( ' masonry_%d', $columns ) : '' )
					: ( woohoo_is_blog_style_use_masonry( $style )
		...
		if ( $args['sticky'] ) {
			...
			if ( woohoo_get_theme_option( 'first_post_large' ) && ! is_paged() && ! in_array( woohoo_get_theme_option( 'body_style' ), array( 'fullwide', 'fullscreen' ) ) && have_posts() ) {
				the_post();
				get_template_part( apply_filters( 'woohoo_filter_get_template_part', 'templates/content', 'excerpt' ), 'excerpt' );
			}
			?>
		...
		while ( have_posts() ) {
			...
			get_template_part( apply_filters( 'woohoo_filter_get_template_part', $args['sticky'] && is_sticky() ? 'templates/content-sticky' : woohoo_blog_item_get_template( $blog_style ) ) );
		}
...
if ( ! function_exists( 'woohoo_ajax_get_posts_callback' ) ) {
	add_action( 'wp_ajax_woohoo_ajax_get_posts', 'woohoo_ajax_get_posts_callback' );
	add_action( 'wp_ajax_nopriv_woohoo_ajax_get_posts', 'woohoo_ajax_get_posts_callback' );
	/**
	 * Return a portfolio posts layout to AJAX requests when switching tabs and pages.
	 *
	 * Hooks: add_action( 'wp_ajax_woohoo_ajax_get_posts', 'woohoo_ajax_get_posts_callback' );
	 * add_action( 'wp_ajax_nopriv_woohoo_ajax_get_posts', 'woohoo_ajax_get_posts_callback' );
	 */
	function woohoo_ajax_get_posts_callback() {
		...
		$response = array(
			...
			'css'  => woohoo_get_inline_css(),
		);

		if ( empty( $response['data'] ) ) {
			$response['error'] = esc_html__( 'Sorry, but nothing matched your search criteria.', 'woohoo' );
		}
...
if ( ! function_exists( 'woohoo_show_pagination' ) ) {
	/**
	 ...
	 */
	function woohoo_show_pagination( $args = array() ) {
		global $wp_query;
		$pagination = ! empty( $args[ 'pagination' ] )
						? $args[ 'pagination' ]
						: woohoo_get_theme_option( 'blog_pagination' );
		$prefix     = ! empty( $args[ 'class_prefix' ] )
						? $args[ 'class_prefix' ]
						: 'nav';
		if ( 'pages' == $pagination ) {
			woohoo_show_layout( str_replace( "\n", '', get_the_posts_pagination(
				apply_filters( 'woohoo_filter_get_the_posts_pagination_args', array(
					'mid_size'           => 2,
					'prev_text'          => esc_html__( '<', 'woohoo' ),
					'next_text'          => esc_html__( '>', 'woohoo' ),
					'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'woohoo' ) . ' </span>',
				) )
			) ) );
			...
			if ( $page_number < $wp_query->max_num_pages ) {
				...
				<div class="<?php echo esc_attr( $prefix ); ?>-links-more
					...
					?>
				">
					<a class="<?php echo esc_attr( apply_filters( 'woohoo_filter_load_more_class', "{$prefix}-load-more" ) ); ?>"
						href="#" 
						data-page="<?php echo esc_attr( $page_number ); ?>" 
						data-max-page="<?php echo esc_attr( $wp_query->max_num_pages ); ?>"
						<?php do_action( 'woohoo_action_load_more_data' ); ?>
						><span><?php echo esc_html( apply_filters( 'woohoo_filter_load_more_text', __( 'Load more', 'woohoo' ) ) ); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="<?php echo esc_attr( $prefix ); ?>-links-old">
				<span class="<?php echo esc_attr( $prefix ); ?>-prev"><?php previous_posts_link( is_search() ? esc_html__( 'Previous posts', 'woohoo' ) : esc_html__( 'Newest posts', 'woohoo' ) ); ?></span>
				<span class="<?php echo esc_attr( $prefix ); ?>-next"><?php next_posts_link( is_search() ? esc_html__( 'Next posts', 'woohoo' ) : esc_html__( 'Older posts', 'woohoo' ), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'woohoo_single_comments_field' ) ) {
	/**
	 ...
	 */
	function woohoo_single_comments_field( $args ) {
		...
		if ( 'text' == $args['field_type'] ) {
			...
			$html .= '<input id="' . esc_attr( $args['field_name'] ) . '" name="' . esc_attr( $args['field_name'] ) . '" type="checkbox" value="' . esc_attr( $args['field_value'] ) . '"' . ( $args['field_req'] ? ' aria-required="true"' : '' ) . ' />'
					. ' <label for="' . esc_attr( $args['field_name'] ) . '" class="' . esc_attr( $args['field_req'] ? 'required' : 'optional' ) . '">' . wp_kses( $args['field_title'], 'woohoo_kses_content' ) . '</label>';
		} else {
		...
		if ( 'default' != $args['form_style'] && in_array( $args['field_type'], array( 'text', 'textarea' ) ) ) {
			$html .= '<span class="sc_form_field_hover">'
						...
						. '<span class="sc_form_field_content" data-content="' . esc_attr( $args['field_title'] ) . '">' . wp_kses( $args['field_title'], 'woohoo_kses_content' ) . '</span>'
					. '</span>';
		}
		$html .= '</span></div>';
		return $html;
