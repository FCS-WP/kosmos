...
if ( ! defined( 'WOOHOO_THEME_FREE' ) ) {
	define( 'WOOHOO_THEME_FREE', false );
}
if ( ! defined( 'WOOHOO_THEME_FREE_WP' ) ) {
	define( 'WOOHOO_THEME_FREE_WP', false );
}

// If this theme support external updates
if ( ! defined( 'WOOHOO_THEME_ALLOW_UPDATE' ) ) {
	define( 'WOOHOO_THEME_ALLOW_UPDATE', true );
}

// If this theme uses skins
if ( ! defined( 'WOOHOO_ALLOW_SKINS' ) ) {
	define( 'WOOHOO_ALLOW_SKINS', true );
}
if ( ! defined( 'WOOHOO_DEFAULT_SKIN' ) ) {
	define( 'WOOHOO_DEFAULT_SKIN', 'default' );
}
if ( ! defined( 'WOOHOO_REMEMBER_SKIN' ) ) {
	define( 'WOOHOO_REMEMBER_SKIN', false );
}
...
if ( ! function_exists( 'woohoo_get_theme_pro_key' ) ) {
	/**
	 ...
	 */
	function woohoo_get_theme_pro_key() {
		$theme_pro_key = get_option( sprintf( 'purchase_code_src_%s', get_template() ) );
		if ( empty( $theme_pro_key ) ) {
			$theme_pro_key = woohoo_storage_get( 'theme_pro_key' );
		}
		return $theme_pro_key;
	}
}

if ( ! function_exists( 'woohoo_trx_updater_get_theme_info' ) ) {
	add_filter( 'trx_updater_filter_get_theme_info', 'woohoo_trx_updater_get_theme_info' );
	/**
	 * Add an information about the theme engine to the array with a theme info.
	 *
	 * Hooks: add_filter( 'trx_updater_filter_get_theme_info', 'woohoo_trx_updater_get_theme_info' );
	 *
	 * @param array $info  An array with a theme information.
	 *
	 * @return array       A theme information with an engine info.
	 */
	function woohoo_trx_updater_get_theme_info( $info ) {
		$engine = woohoo_storage_get( 'engine' );
...
if ( ! function_exists( 'woohoo_original_theme_slug' ) ) {
	add_filter( 'woohoo_filter_original_theme_slug', 'woohoo_original_theme_slug' );
	/**
	 ...
	 *
	 * Hooks: add_filter( 'woohoo_filter_original_theme_slug', 'woohoo_original_theme_slug' );
	 *
	 * @param string $theme_slug  Current slug of the theme.
	 *
	 * @return string             Original slug of the theme.
	 */
	function woohoo_original_theme_slug( $theme_slug = '' ) {
		return 'woohoo';
	}
}
...
if ( ! function_exists( 'woohoo_importer_set_options' ) ) {
	add_filter( 'trx_addons_filter_importer_options', 'woohoo_importer_set_options', 9 );
	/**
	 * Set a theme-specific options for the import of the demo data via plugin ThemeREX Addons.
	 *
	 * Hooks: add_filter( 'trx_addons_filter_importer_options', 'woohoo_importer_set_options', 9 );
	 *
	 * @param array $options  Options for the import of the demo data.
	 *
	 * @return array          Processed options with a theme-specific values.
	 */
	function woohoo_importer_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			// Prepare demo data
			$options['demo_url'] = esc_url( woohoo_get_protocol() . ':' . woohoo_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
			$options['required_plugins'] = array_keys( woohoo_storage_get( 'required_plugins' ) );
...
if ( ! function_exists( 'woohoo_ocdi_set_options' ) ) {
	add_filter( 'trx_addons_filter_ocdi_options', 'woohoo_ocdi_set_options', 9 );
	/**
	 ...
	 * @return array          Processed options with a theme-specific values.
	 */
	function woohoo_ocdi_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			// Prepare demo data
			$options['demo_url'] = esc_url( woohoo_get_protocol() . ':' . woohoo_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
...
if ( ! function_exists( 'woohoo_customizer_theme_setup1' ) ) {
	add_action( 'after_setup_theme', 'woohoo_customizer_theme_setup1', 1 );
	/**
	 * Create an internal setting for the theme.
	 * Attention! This settings ONLY FOR PROGRAMMERS, NOT FOR CUSTOMER
	 *
	 * Hooks: add_action( 'after_setup_theme', 'woohoo_customizer_theme_setup1', 1 );
	 *
	 ...
	 */
	function woohoo_customizer_theme_setup1() {
		woohoo_storage_set(
			'settings', array(
				...
				'allow_fullscreen'          => false,                   // Allow cases 'fullscreen' and 'fullwide' for the body style in the Theme Options
																		...
																		// (can be removed if filter 'woohoo_filter_allow_fullscreen' return false)
