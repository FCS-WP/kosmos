...
( function( $ ) {
	...
	var $style = $( '#woohoo-customizer-inline-css' ),
		api    = wp.customize;

	// Prepare inline styles in the preview window
	if ( $style.length === 0 ) {
		var style_container = '<' + 'style type="text/css" id="woohoo-customizer-inline-css" />',
			$theme_css      = $( '#woohoo-custom-inline-css' );
		if ($theme_css.length === 0) {
			$theme_css = $( '#woohoo-custom-css' );
		}
		if ($theme_css.length === 0) {
		...
		$style = $( '#woohoo-customizer-inline-css' );
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-other-controls', function( obj ) {
			...
			if (id.indexOf( 'body_style' ) === 0) {
				...
				$( '.sc_layouts_logo .logo_text' ).html( woohoo_prepare_macros( val ) );

			} else if (id == 'blogdescription') {
				$( '.sc_layouts_logo .logo_slogan' ).html( woohoo_prepare_macros( val ) );

			} else if (id == 'copyright') {
				$( '.copyright_text' ).html( woohoo_prepare_macros( val ) );
				...
				if (id == 'front_page_bg_image') {
					...
					if (color_type == 'none') {
						...
						color = typeof woohoo_color_schemes[scheme] != 'undefined' ? woohoo_color_schemes[scheme]['colors']['bg_color'] : '';
					}
					$( '.front_page_section_' + section + '_inner' ).css( 'background-color', color == '' ? 'transparent' : ( mask == 1 ? color : woohoo_hex2rgba( color, mask ) ) );
					...
					.height( 'auto' )
					.toggleClass( 'woohoo-full-height sc_layouts_flex sc_layouts_columns_middle', val );
					$( window ).trigger( 'resize' );
				}
			}
		} );
		...
		function add_color_scheme_helpers() {
			$( '[class*="scheme_"]:not(html)' ).each( function() {
				var $self = $( this ),
					...
					scheme = woohoo_get_class_by_prefix( classes, 'scheme_' ).replace( 'scheme_', '' );
				$self.append( '<div class="woohoo_customizer_scheme_helper">'
									+ '<span class="woohoo_customizer_scheme_helper_name">'
										+ tag
										+ ( id
											? '#' + id
											: ( classes
		...
		function remove_color_scheme_helpers() {
			$( '.woohoo_customizer_scheme_helper' ).remove();
		}
