...
if ( function_exists( 'woohoo_elementor_is_preview' ) && woohoo_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			woohoo_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	woohoo_new_main_query(
		array(
			'post_type'      => woohoo_get_theme_option( 'post_type' ),
			'category'       => woohoo_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'woohoo_filter_get_template_part', woohoo_blog_archive_get_template() ) );
}
