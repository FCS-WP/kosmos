...
//							woohoo_document_animate_to('#content_skip_link_anchor');
							...
							setTimeout( function() {
								panel
									.find( '.woohoo_tab_holder,.woohoo_tab_content_remove' ).remove();
							}, 300 );
						}
					);
		...
		if ( WOOHOO_STORAGE['resize_tag_iframe'] ) {
			cont.find( '.video_frame iframe,iframe' ).each( function() {
				...
				if ( $self.hasClass( 'trx_addons_resize' )
					...
					|| $self.addClass( 'woohoo_resize' ).parents( 'div:hidden,section:hidden,article:hidden' ).length > 0
				) {
					return;
				}
				...
				if ( ! w_attr || ! h_attr || w_attr <= woohoo_apply_filters( 'woohoo_filter_noresize_iframe_width', 325 ) ) {
					return;
				}
