...
if ( ! function_exists( 'woohoo_wp_theme_setup' ) ) {

	add_action( 'after_setup_theme', 'woohoo_wp_theme_setup' );
	 ...
	 */
	function woohoo_wp_theme_setup() {

		// Remove macros from title
		add_filter( 'wp_title', 'woohoo_remove_macros_from_site_title' );
		add_filter( 'wp_title_parts', 'woohoo_remove_macros_from_site_title' );
		add_filter( 'document_title_parts', 'woohoo_remove_macros_from_site_title' );

		// Breadcrumbs link 'All posts'
		add_filter( 'post_type_archive_link', 'woohoo_get_template_page_link', 10, 2 );
	}
}


if ( ! function_exists( 'woohoo_is_preview' ) ) {
	/**
	 ...
	 */
	function woohoo_is_preview( $builder = 'any' ) {
		return ( in_array( $builder, array( 'any', 'elm', 'elementor' ) ) && function_exists( 'woohoo_elementor_is_preview' ) && woohoo_elementor_is_preview() )
				||
				( in_array( $builder, array( 'any', 'gb', 'gutenberg' ) ) && function_exists( 'woohoo_gutenberg_is_preview' ) && woohoo_gutenberg_is_preview() );
	}
}


if ( ! function_exists( 'woohoo_get_wp_template_hooks' ) ) {
	/**
	 ...
	 */
	function woohoo_get_wp_template_hooks() {
		return apply_filters( 'woohoo_filter_wp_template_hooks', array(
					'404_template',
					'archive_template',
					'attachment_template',
					'author_template',
...
if ( ! function_exists( 'woohoo_detect_blog_mode' ) ) {
	/**
	 ...
	 * 
	 * For example:
	 *   $pos = woohoo_get_theme_option( 'sidebar_position' );
	 * 
	 ...
	 */
	function woohoo_detect_blog_mode() {
		if ( is_front_page() && ! is_home() ) {
			$mode = 'front';
		} elseif ( is_home() ) {
			$mode = 'home';     // Specify 'blog' if you don't need a separate options for the homepage
		} elseif ( woohoo_is_single() ) {
			$mode = 'post';
		} elseif ( is_page() && ! woohoo_storage_isset( 'blog_archive' ) ) {
			$mode = 'page';
		} elseif ( is_category() ) {
			$mode = 'category';
		} elseif ( is_tag() ) {
		...
		return apply_filters( 'woohoo_filter_detect_blog_mode', $mode );
	}
}

if ( ! function_exists( 'woohoo_is_blog_mode_custom' ) ) {
	/**
	 ...
	 */
	function woohoo_is_blog_mode_custom() {
		return ! in_array( woohoo_storage_get( 'blog_mode' ), apply_filters( 'woohoo_filter_blog_mode_standard', array(
...
if ( ! function_exists( 'woohoo_get_current_mode_image' ) ) {
	/**
	 ...
	 */
	function woohoo_get_current_mode_image( $default = '' ) {
		if ( is_category() || is_tax() ) {
			$img = woohoo_get_term_image();
			if ( '' != $img ) {
				$default = $img;
			}
		} elseif ( woohoo_is_singular() || woohoo_storage_isset( 'blog_archive' ) ) {
			if ( is_home() ) {
		...
		return woohoo_clear_thumb_size( $default );
	}
}

if ( ! function_exists( 'woohoo_blog_archive_start' ) ) {
	/**
	 ...
	 */
	function woohoo_blog_archive_start() {
		$main_post = woohoo_storage_get( 'blog_archive_template_post' );
		if ( is_object( $main_post ) ) {
			...
			$woohoo_content            = '';
			$woohoo_blog_archive_mask  = '%%CONTENT%%';
			$woohoo_blog_archive_subst = sprintf( '<div class="blog_archive">%s</div>', $woohoo_blog_archive_mask );
			$woohoo_content            = apply_filters( 'the_content', get_the_content() );
			...
			if ( '' != $woohoo_content ) {
				$woohoo_pos = stripos( $woohoo_content, $woohoo_blog_archive_mask );
				if ( false !== $woohoo_pos ) {
					$woohoo_content = preg_replace( '/(\<p\>\s*)?' . $woohoo_blog_archive_mask . '(\s*\<\/p\>)/i', $woohoo_blog_archive_subst, $woohoo_content );
				} else {
					$woohoo_content .= $woohoo_blog_archive_subst;
				}
				$woohoo_content = explode( $woohoo_blog_archive_mask, $woohoo_content );
				// Display first part
				woohoo_show_layout( apply_filters( 'woohoo_filter_blog_archive_start', $woohoo_content[0] ) );
				// And store second part
				woohoo_storage_set( 'blog_archive_end', $woohoo_content[1] );
			}
...
if ( ! function_exists( 'woohoo_blog_archive_end' ) ) {
	/**
	 * Display the end blog archive template.
	 * 
	 * Output a second part (stored in the previous call of woohoo_blog_archive_start).
	 */
	function woohoo_blog_archive_end() {
		$html = woohoo_storage_get( 'blog_archive_end' );
		if ( '' != $html ) {
			// Display second part of template content
			woohoo_show_layout( apply_filters( 'woohoo_filter_blog_archive_end', $html ) );
		}
	}
}

if ( ! function_exists( 'woohoo_blog_archive_get_template' ) ) {
	/**
	 ...
	 */
	function woohoo_blog_archive_get_template( $blog_style = '' ) {
		if ( empty( $blog_style ) ) {
			$blog_style = woohoo_get_theme_option( 'blog_style' );
		}
		$parts   = explode( '_', $blog_style );
		$archive = 'index';
		if ( woohoo_storage_isset( 'blog_styles', $parts[0], 'archive' ) ) {
			$archive = woohoo_storage_get_array( 'blog_styles', $parts[0], 'archive' );
		}
		return apply_filters( 'woohoo_filter_blog_archive_template', $archive, $blog_style );
	}
}

if ( ! function_exists( 'woohoo_blog_item_get_template' ) ) {
	/**
	 ...
	 *                           or an empty string if not detected.
	 */
	function woohoo_blog_item_get_template( $blog_style = '' ) {
		if ( empty( $blog_style ) ) {
			$blog_style = woohoo_get_theme_option( 'blog_style' );
		}
		$parts = explode( '_', $blog_style );
		...
		} elseif ( woohoo_storage_isset( 'blog_styles', $parts[0], 'item' ) ) {
			$item = woohoo_storage_get_array( 'blog_styles', $parts[0], 'item' );
		} else {
...
if ( ! function_exists( 'woohoo_get_post_id' ) ) {
	/**
	 ...
	 */
	function woohoo_get_post_id( $args = array() ) {
		$args  = array_merge(
			array(
				'posts_per_page' => 1,
			), $args
...
if ( ! function_exists( 'woohoo_get_post_content' ) ) {
	/**
	 ...
	 */
	function woohoo_get_post_content( $apply_filters = false ) {
		global $post;
...
if ( ! function_exists( 'woohoo_filter_post_content' ) ) {
	/**
	 ...
	 */
	function woohoo_filter_post_content( $content ) {
		$content = apply_filters( 'woohoo_filter_post_content', $content );
		global $wp_embed;
...
if ( ! function_exists( 'woohoo_get_template_page_id' ) ) {
	/**
	 ...
	 */
	function woohoo_get_template_page_id( $args = array() ) {
		$args   = array_merge(
			array(
				'template'   => 'blog.php',
				'post_type'  => 'post',
		...
		if ( '' !== $args['parent_cat'] ) {
			$q_args['meta_query'][] = array(
				'key'     => 'woohoo_options_parent_cat',
				'value'   => $args['parent_cat'] > 0 ? $args['parent_cat'] : 1,
				'compare' => $args['parent_cat'] > 0 ? '=' : '<',
			);
		}
		return woohoo_get_post_id( $q_args );
	}
}

if ( ! function_exists( 'woohoo_get_template_page_link' ) ) {
	/**
	 ...
	 * 
	 * Handler of the add_filter('post_type_archive_link', 'woohoo_get_template_page_link', 10, 2 );
	 * 
	 ...
	 */
	function woohoo_get_template_page_link( $link = '', $post_type = '' ) {
		if ( ! empty( $post_type ) ) {
			$id = woohoo_get_template_page_id(
				array(
					'post_type'  => $post_type,
					'parent_cat' => 0,
				)
...
if ( ! function_exists( 'woohoo_get_posts_archive_template' ) ) {

	add_filter( 'archive_template', 'woohoo_get_posts_archive_template', 100 );
	 ...
	 */
	function woohoo_get_posts_archive_template( $template ) {
		if ( woohoo_get_theme_option( 'use_blog_archive_pages' ) ) {
			if ( is_post_type_archive() ) {
				$obj = get_queried_object();
				if ( ! empty( $obj->name ) ) {
					$templates = get_option( 'woohoo_blog_archive_templates' );
					if ( ! empty( $templates[ $obj->name ] ) ) {
						$template = woohoo_redirect_to_archive_template( $template, $templates[ $obj->name ] );
					}
				}
			} else {
				$template = woohoo_get_tax_archive_template( $template );
			}
		}
		return $template;
	}	
...
if ( ! function_exists( 'woohoo_get_tax_archive_template' ) ) {

	add_filter( 'category_template', 'woohoo_get_tax_archive_template', 100 );
	add_filter( 'taxonomy_template', 'woohoo_get_tax_archive_template', 100 );
	 ...
	 * @return string          A filtered path to the template
	 */
	function woohoo_get_tax_archive_template( $template ) {
		if ( woohoo_get_theme_option( 'use_blog_archive_pages' ) && ( is_category() || is_tag() || is_tax() ) ) {
			$obj = get_queried_object();
			global $wp_query;
			...
			if ( $pt && $tax && $term ) {
				$pt_tax  = woohoo_get_post_type_taxonomy( $pt );
				if ( $pt_tax == $tax ) {
					$templates = get_option( 'woohoo_blog_archive_templates' );
					if ( ! empty( $templates[ "{$pt}_{$tax}_{$term}" ] ) ) {
						$template = woohoo_redirect_to_archive_template( $template, $templates[ "{$pt}_{$tax}_{$term}" ] );
					} else {
						$found = false;
						do {
							...
							if ( ! empty( $templates[ "{$pt}_{$tax}_{$parent}" ] ) ) {
								$template = woohoo_redirect_to_archive_template( $template, $templates[ "{$pt}_{$tax}_{$parent}" ], $term );
								$found = true;
								break;
							} else {
						...
						if ( ! $found && ! empty( $templates[ "{$pt}" ] ) ) {
							$template = woohoo_redirect_to_archive_template( $template, $templates[ "{$pt}" ], $term );
						}
					}
				}
			}
...
if ( ! function_exists( 'woohoo_redirect_to_archive_template' ) ) {
	/**
	 ...
	 */
	function woohoo_redirect_to_archive_template( $template, $page_id, $term = false ) {
		// Store page number
		...
		// Load page options
		woohoo_override_theme_options( null, $page_id );
		// Override parent category
		if ( $term > 0 ) {
			woohoo_storage_set_array( 'options_meta', 'parent_cat', $term );
		}
		return woohoo_get_file_dir( 'blog.php' );
	}
}

if ( ! function_exists( 'woohoo_save_archive_template' ) ) {

	add_action( 'save_post', 'woohoo_save_archive_template', 11 );
	 ...
	 */
	function woohoo_save_archive_template( $post_id ) {
		// verify nonce
		if ( ! wp_verify_nonce( woohoo_get_value_gp( 'override_options_nonce' ), admin_url() ) ) {
			return $post_id;
		}
		...
		if ( 'page' == $post_type ) {
			$meta = get_post_meta( $post_id, 'woohoo_options', true );
			...
			if ( 'blog.php' == $page_template ) {
				...
				$tax  = woohoo_get_post_type_taxonomy( $pt );
				$term = isset( $meta['parent_cat'] ) ? $meta['parent_cat'] : 0;
				$templates = woohoo_array_delete_by_value( $templates, $post_id );
				$templates[ $pt . ( $term > 0 ? "_{$tax}_{$term}" : '' ) ] = $post_id;
			} else {
				$templates = woohoo_array_delete_by_value( $templates, $post_id );
			}
			update_option( 'woohoo_blog_archive_templates', $templates );
		}
	}
}

// Delete page from templates
if ( ! function_exists( 'woohoo_delete_archive_template' ) ) {

	add_action( 'delete_post', 'woohoo_delete_archive_template', 11 );
	 ...
	 */
	function woohoo_delete_archive_template( $post_id ) {
		$templates = get_option( 'woohoo_blog_archive_templates' );
		if ( is_array( $templates ) ) {
			$templates = woohoo_array_delete_by_value( $templates, $post_id );
			update_option( 'woohoo_blog_archive_templates', $templates );
...
if ( ! function_exists( 'woohoo_get_hash_link' ) ) {
	 ...
	 */
	function woohoo_get_hash_link( $hash ) {
		if ( 0 !== strpos( $hash, 'http' ) ) {
			if ( '#' != $hash[0] ) {
				$hash = '#' . $hash;
			}
			if ( is_customize_preview() ) {
				$url = woohoo_get_current_url();
				$pos = strpos( $url, '#' );
				if ( false !== $pos ) {
...
if ( ! function_exists( 'woohoo_get_current_url' ) ) {
	/**
	 * Return an URL to the current page.
	 * 
	 * @return string  A current page URL.
	 */
	function woohoo_get_current_url() {
		return add_query_arg( array() );
	}
}

if ( ! function_exists( 'woohoo_check_url' ) ) {
	/**
...
if ( ! function_exists( 'woohoo_attachment_url_to_postid' ) ) {
	/**
	 ...
	 */
	function woohoo_attachment_url_to_postid( $url ) {
		static $images = array();
		if ( ! isset( $images[ $url ] ) ) {
			$images[ $url ] = attachment_url_to_postid( woohoo_clear_thumb_size( $url, false ) );
		}
		return $images[ $url ];
	}
}

if ( ! function_exists( 'woohoo_remove_macros_from_site_title' ) ) {
	/**
	 ...
	 */
	function woohoo_remove_macros_from_site_title( $title ) {
		if ( is_array( $title ) ) {
			foreach ( $title as $k => $v ) {
				$title[ $k ] = woohoo_remove_macros( $v );
			}
		} else {
			$title = woohoo_remove_macros( $title );
		}
		return $title;
	}
}

if ( ! function_exists( 'woohoo_get_blog_title' ) ) {
	/**
	 * Return a blog title for the current blog mode.
	 *
	 * @return string  A blog title.
	 */
	function woohoo_get_blog_title() {

		if ( is_front_page() ) {
			$title = esc_html__( 'Home', 'woohoo' );
		} elseif ( is_home() ) {
			$title = esc_html__( 'All Posts', 'woohoo' );
		} elseif ( is_author() ) {
			$curauth = ( get_query_var( 'author_name' ) ) ? get_user_by( 'slug', get_query_var( 'author_name' ) ) : get_userdata( get_query_var( 'author' ) );
			// Translators: Add the author's name to the title
			$title = sprintf( esc_html__( 'Author page: %s', 'woohoo' ), $curauth->display_name );
		} elseif ( is_404() ) {
			$title = esc_html__( 'URL not found', 'woohoo' );
		} elseif ( is_search() ) {
			// Translators: Add the author's name to the title
			$title = sprintf( esc_html__( 'Search: %s', 'woohoo' ), get_search_query() );
		} elseif ( is_day() ) {
			// Translators: Add the queried date to the title
			$title = sprintf( esc_html__( 'Daily Archives: %s', 'woohoo' ), get_the_date() );
		} elseif ( is_month() ) {
			// Translators: Add the queried month to the title
			$title = sprintf( esc_html__( 'Monthly Archives: %s', 'woohoo' ), get_the_date( 'F Y' ) );
		} elseif ( is_year() ) {
			// Translators: Add the queried year to the title
			$title = sprintf( esc_html__( 'Yearly Archives: %s', 'woohoo' ), get_the_date( 'Y' ) );
		} elseif ( is_category() ) {
			$title = single_cat_title( '', false );
		} elseif ( is_tag() ) {
			// Translators: Add the tag's name to the title
			$title = sprintf( esc_html__( 'Tag: %s', 'woohoo' ), single_tag_title( '', false ) );
		} elseif ( is_tax() ) {
			...
			$title = sprintf( esc_html__( 'Attachment: %s', 'woohoo' ), get_the_title() );
		} elseif ( woohoo_is_single() || is_page() ) {
			$title = get_the_title();
		} else {
			$title = get_the_title();
		}
		return apply_filters( 'woohoo_filter_get_blog_title', $title );
	}
}

if ( ! function_exists( 'woohoo_get_post_categories' ) ) {
	/**
	 ...
	 */
	function woohoo_get_post_categories( $delimiter = ', ', $id = false, $links = true ) {
		return woohoo_get_post_terms( $delimiter, $id, 'category', $links );
	}
}

if ( ! function_exists( 'woohoo_get_post_terms' ) ) {
	/**
	 ...
	 */
	function woohoo_get_post_terms( $delimiter = ', ', $id = false, $taxonomy = 'category', $links = true ) {
		$output = '';
		if ( empty( $id ) ) {
			$id = get_the_ID();
		}
		if ( empty( $taxonomy ) ) {
			$taxonomy = woohoo_get_post_type_taxonomy( get_post_type( $id ) );
		}
				...
				$output .= ( ! empty( $output )
							...
							. ( $links
								? '<a href="' . esc_url( get_term_link( $term->term_id, $taxonomy ) ) . '"'
										...
										. ' title="' . sprintf( esc_attr__( 'View all posts in %s', 'woohoo' ), $term->name ) . '"'
										. '>'
								: '<span>'
								)
								. apply_filters( 'woohoo_filter_term_name', $term->name, $term )
							. ( $links
								? '</a>'
								: '</span>'
								);
...
if ( ! function_exists( 'woohoo_get_post_type_taxonomy' ) ) {
	/**
	 ...
	 */
	function woohoo_get_post_type_taxonomy( $post_type = '' ) {
		if ( empty( $post_type ) ) {
		...
		return apply_filters( 'woohoo_filter_post_type_taxonomy', $tax, $post_type );
	}
}

if ( ! function_exists( 'woohoo_get_edited_post_id' ) ) {
	/**
	 ...
	 */
	function woohoo_get_edited_post_id() {
		$id = false;
		if ( is_admin() ) {
			$url = woohoo_get_current_url();
			if ( strpos( $url, 'post.php' ) !== false ) {
				if ( woohoo_get_value_gp( 'action' ) == 'edit' ) {
					$post_id = woohoo_get_value_gp( 'post' );
					if ( 0 < $post_id ) {
						$id = $post_id;
					}
				}
...
if ( ! function_exists( 'woohoo_get_edited_post_type' ) ) {
	/**
	 ...
	 */
	function woohoo_get_edited_post_type() {
		$pt = '';
		if ( is_admin() ) {
			$url = woohoo_get_current_url();
			if ( strpos( $url, 'post.php' ) !== false ) {
				if ( in_array( woohoo_get_value_gp( 'action' ), array( 'edit', 'elementor' ) ) ) {
					$id = woohoo_get_value_gp( 'post' );
					if ( 0 < $id ) {
				...
				$pt = woohoo_get_value_gp( 'post_type' );
			}
		}
		return $pt;
	}
...
if ( ! function_exists( 'woohoo_is_post_edit' ) ) {
	/**
	 * Detect if a current mode is "Edit post" or "Site editor".
	 *
	 * @return bool Return true if current mode is "Edit post" or "Edit site".
	 */
	function woohoo_is_post_edit() {
		return ( woohoo_check_url( 'post.php' ) && ! empty( $_GET['action'] ) && $_GET['action'] == 'edit')
				||
				woohoo_check_url( 'post-new.php' )
				||
				( woohoo_check_url( '/block-renderer/trx-addons/' ) && ! empty( $_GET['context'] ) && $_GET['context'] == 'edit' )
				||
				( woohoo_check_url( 'admin.php' ) && ! empty( $_GET['page'] ) && $_GET['page'] == 'gutenberg-edit-site' )
				||
				( woohoo_check_url( 'site-editor.php' ) && ! empty( $_GET['postType'] ) )	// || $_GET['postType'] == 'wp_template' ) )
				||
				woohoo_check_url( 'widgets.php' );
	}
}

if ( ! function_exists( 'woohoo_is_singular' ) ) {
	/**
	 ...
	 */
	function woohoo_is_singular( $type = '' ) {
		global $wp_query;
		return apply_filters( 'woohoo_filter_is_singular', ! empty( $wp_query->queried_object->ID ) && is_singular( $type ), $type );
	}
}

if ( ! function_exists( 'woohoo_is_single' ) ) {
	/**
	 * Detect if the current post is a single post.
	 *
	 * @return bool  true if the current mode is a single post.
	 */
	function woohoo_is_single() {
		global $wp_query;
		return apply_filters( 'woohoo_filter_is_single', ! empty( $wp_query->queried_object->ID ) && is_single() );
	}
}

if ( ! function_exists( 'woohoo_add_seo_itemprops' ) ) {
	/**
	 * Add SEO params to the article tag.
	 */
	function woohoo_add_seo_itemprops() {
		if ( woohoo_is_on( woohoo_get_theme_option( 'seo_snippets' ) ) ) {
			...
			?>" 
			itemtype="<?php echo esc_attr( woohoo_get_protocol( true ) ); ?>//schema.org/<?php echo esc_attr( woohoo_get_markup_schema() ); ?>" 
...
if ( ! function_exists( 'woohoo_add_seo_snippets' ) ) {
	/**
	 * Add SEO meta to the post from the template 'templates/seo.php'
	 */
	function woohoo_add_seo_snippets() {
		if ( woohoo_is_on( woohoo_get_theme_option( 'seo_snippets' ) ) ) {
...
if ( ! function_exists( 'woohoo_get_nav_menu' ) ) {
	/**
	 ...
	 * 									  or true to use a custom walker from the theme with class 'woohoo_custom_menu_walker'.
	 * 									  Default is false.
	 *
	 * @return string               A menu layout or an error message.
	 */
	function woohoo_get_nav_menu( $location = '', $menu = '', $depth = 0, $custom_walker = false ) {
		static $list = array();
		$slug = $location . '_' . $menu;
		if ( empty( $list[ $slug ] ) ) {
			$list[ $slug ] = esc_html__( 'You are trying to use a menu inserted in himself!', 'woohoo' );
			$args          = array(
								'menu'            => empty( $menu ) || 'default' == $menu || woohoo_is_inherit( $menu ) ? '' : $menu,
								'container'       => '',
								'container_class' => '',
								'container_id'    => '',
								...
								'before'          => '',
								'after'           => '',
								'link_before'     => woohoo_get_theme_setting( 'wrap_menu_items_with_span' ) ? '<span>' : '',
								'link_after'      => woohoo_get_theme_setting( 'wrap_menu_items_with_span' ) ? '</span>' : '',
								'depth'           => $depth,
							);
			if ( ! empty( $location ) ) {
			...
			if ( $custom_walker ) {
				...
				} else if ( $custom_walker === true && class_exists( 'woohoo_custom_menu_walker' ) ) {
					$args['walker'] = new woohoo_custom_menu_walker;
				}
			}
			$list[ $slug ] = wp_nav_menu( apply_filters( 'woohoo_filter_get_nav_menu_args', $args ) );
		}
		return apply_filters( 'woohoo_filter_get_nav_menu', $list[ $slug ], $location, $menu );
	}
}

if ( ! function_exists( 'woohoo_remove_empty_spaces_between_menu_items' ) ) {

    add_action( 'wp_nav_menu', 'woohoo_remove_empty_spaces_between_menu_items', 98, 2 );
	 ...
	 */
	function woohoo_remove_empty_spaces_between_menu_items( $html = '', $args = array() ) {
		return preg_replace(
							array( "/>[\r\n\s]*<li/", "/>[\r\n\s]*<\\/ul>/" ),
							array( "><li", "></ul>" ),
							$html
...
if ( ! function_exists( 'woohoo_remove_empty_menu_items' ) ) {

	add_action( 'wp_nav_menu', 'woohoo_remove_empty_menu_items', 99, 2 );
	 ...
	 * @return string       A clean string.
	 */
	function woohoo_remove_empty_menu_items( $html = '', $args = array() ) {
		return woohoo_get_theme_setting( 'remove_empty_menu_items' )
					? preg_replace(
							"/<li[^>]*>[\r\n\s]*<a[^>]*>[\r\n\s]*(<span>[\r\n\s]*<\\/span>[\r\n\s]*)?<\\/a>[\r\n\s]*<\\/li>/",
							"",
							$html
...
//
if ( ! function_exists( 'woohoo_new_main_query' ) ) {
	/**
	 ...
	 */
	function woohoo_new_main_query( $args ) {
		...
		if ( ! empty( $args['post_type'] ) || ! empty( $args['category'] ) ) {
			$query_args  = woohoo_query_add_posts_and_cats( $query_args, $args['post_ids'], $args['post_type'], $args['category'] );
		}
		if ( $args[ 'page' ] > 1 ) {
...
if ( ! function_exists( 'woohoo_query_add_sort_order' ) ) {
	/**
	 ...
	 */
	function woohoo_query_add_sort_order( $args, $orderby = 'date', $order = 'desc' ) {
		if ( ! empty( $orderby ) && ( empty( $args['orderby'] ) || 'none' != $orderby ) ) {
			$q          = apply_filters( 'woohoo_filter_add_sort_order', array(), $orderby, $order );
		...
		return apply_filters( 'woohoo_filter_add_sort_order_args', $args, $orderby, $order );
	}
}

if ( ! function_exists( 'woohoo_query_add_posts_and_cats' ) ) {
	/**
	 ...
	 */
	function woohoo_query_add_posts_and_cats( $args, $ids = '', $post_type = '', $cat = '', $taxonomy = '' ) {
		if ( ! empty( $ids ) ) {
			...
			if ( ! empty( $cat ) ) {
				...
				if ( empty( $taxonomy ) ) {
					$taxonomy = woohoo_get_post_type_taxonomy( $post_type );
				}
...
if ( ! function_exists( 'woohoo_query_add_filters' ) ) {
	/**
	 ...
	 */
	function woohoo_query_add_filters( $args, $filters = false ) {
		if ( ! empty( $filters ) ) {
			if ( ! is_array( $filters ) ) {
...
if ( ! function_exists( 'woohoo_create_widgets_area' ) ) {
	/**
	 ...
	 */
	function woohoo_create_widgets_area( $name, $add_classes = '' ) {
		$widgets_name = woohoo_get_theme_option( $name );
		if ( ! woohoo_is_off( $widgets_name ) && is_active_sidebar( $widgets_name ) ) {
			woohoo_storage_set( 'current_sidebar', $name );
			ob_start();
			...
			if ( ! empty( $out ) ) {
				...
				$need_columns = strpos( $out, 'columns_wrap' ) === false && apply_filters( 'woohoo_filter_widgets_area_need_columns', 'footer' == $name, $name, $out );
				if ( $need_columns ) {
					$columns = apply_filters( 'woohoo_filter_widgets_area_columns', min( 4, max( 1, woohoo_tags_count( $out, 'aside' ) ) ), $name );
				...
				<div class="<?php echo esc_attr( $name ); ?> <?php echo esc_attr( $name ); ?>_wrap widget_area">
					<?php do_action( 'woohoo_action_before_sidebar_wrap' ); ?>
					<div class="<?php echo esc_attr( $name ); ?>_inner <?php echo esc_attr( $name ); ?>_inner widget_area_inner">
						<?php
						do_action( 'woohoo_action_before_sidebar' );
						woohoo_show_layout(
							$out,
							true == $need_columns ? '<div class="columns_wrap">' : '',
							true == $need_columns ? '</div>' : ''
						);
						do_action( 'woohoo_action_after_sidebar' );
						?>
					</div>
					<?php do_action( 'woohoo_action_after_sidebar_wrap' ); ?>
				</div>
				<?php
			}
		}
...
if ( ! function_exists( 'woohoo_sidebar_present' ) ) {
	/**
	 ...
	 */
	function woohoo_sidebar_present() {
		global $wp_query;
		$sidebar_position = woohoo_get_theme_option( 'sidebar_position' );
		$sidebar_type     = woohoo_get_theme_option( 'sidebar_type' );
		$sidebar_name     = woohoo_get_theme_option( 'sidebar_widgets' );
		return apply_filters(
			'woohoo_filter_sidebar_present',
			! woohoo_is_off( $sidebar_position )
				&& (
						( 'default' == $sidebar_type && ! woohoo_is_off( $sidebar_name ) && is_active_sidebar( $sidebar_name ) )
						||
						( 'custom' == $sidebar_type && woohoo_is_layouts_available() )
					)
				&& ! is_404()
...
if ( ! function_exists( 'woohoo_get_content_width' ) ) {
	/**
	 ...
	 */
	function woohoo_get_content_width() {
		$pg_width = apply_filters( 'woohoo_filter_content_width', woohoo_get_theme_option( 'page_width' ) );
		$sb_width = 0;
		$sb_gap   = 0;
		if ( woohoo_sidebar_present() ) {
			...
			if ( 'default' == $woohoo_sidebar_type ) {
				...
				if ( is_active_sidebar( $woohoo_sidebar_name ) ) {
					$sb_width = woohoo_get_theme_option( 'sidebar_width' );
					$sb_gap   = woohoo_get_theme_option( 'sidebar_gap' );
				}
			} else {
				$sb_width = woohoo_get_theme_option( 'sidebar_width' );
				$sb_gap   = woohoo_get_theme_option( 'sidebar_gap' );
...
if ( ! function_exists( 'woohoo_add_inline_css_class' ) ) {
	/**
	 ...
	 */
	function woohoo_add_inline_css_class( $css, $suffix = '' ) {
		$class_name = woohoo_generate_id( 'woohoo_inline_' );
		woohoo_add_inline_css(
			sprintf(
				'.%s%s{%s}',
				$class_name,
				! empty( $suffix )
...
if ( ! function_exists( 'woohoo_add_inline_css' ) ) {
	/**
...
------------------------------------------------------------------------------------- */
if ( ! function_exists( 'woohoo_enqueue_styles' ) ) {
	/**
	 * Enqueue a list of styles
	 *
	 * @param array $list - styles to enqueue
	 */
	function woohoo_enqueue_styles( $list, $sc ) {
		if ( is_array( $list ) ) {
			foreach( $list as $handle => $data ) {
				$lib_url = woohoo_get_file_url( $data['src'] );
				if ( $lib_url ) {
					wp_enqueue_style(
						$handle,
						$lib_url,
						...
						! empty( $data['media'] ) ? woohoo_media_for_load_css_responsive( str_replace( '_', '-', $sc ), $data['media'] ) : 'all'
					);
				}
			}
		}
...
if ( ! function_exists( 'woohoo_enqueue_scripts' ) ) {
	/**
	 * Enqueue a list of scripts
	 *
	 * @param array $list - scripts to enqueue
	 */
	function woohoo_enqueue_scripts( $list, $sc ) {
		if ( is_array( $list ) ) {
			foreach( $list as $handle => $data ) {
				$lib_url = woohoo_get_file_url( $data['src'] );
				if ( $lib_url ) {
...
if ( ! function_exists( 'woohoo_enqueue_optimized' ) ) {
	/**
	 ...
	 */
	function woohoo_enqueue_optimized( $sc, $force, $args ) {
		static $loaded = array();
		if ( empty( $loaded[ $sc ] ) && (
			current_action() == 'wp_enqueue_scripts' && woohoo_need_frontend_scripts( ! empty( $args['slug'] ) ? $args['slug'] :  $sc )
			||
			...
			if ( ! isset( $args['need'] ) || $args['need'] ) {
				if ( ! empty( $args['css'] ) ) {
					woohoo_enqueue_styles( $args['css'], $sc );
				}
				if ( ! empty( $args['js'] ) ) {
					woohoo_enqueue_scripts( $args['js'], $sc );
				}
...
if ( ! function_exists( 'woohoo_enqueue_optimized_responsive' ) ) {
	/**
	 ...
	 * @param array $args - arguments with styles and scripts to enqueue
	 */
	function woohoo_enqueue_optimized_responsive( $sc, $force, $args ) {
		static $loaded = array();
		if ( empty( $loaded[ $sc ] ) && (
			current_action() == 'wp_enqueue_scripts' && woohoo_need_frontend_scripts( ! empty( $args['slug'] ) ? $args['slug'] :  $sc )
			||
...
if ( ! function_exists( 'woohoo_get_date' ) ) {
	/**
	 ...
	 */
	function woohoo_get_date( $dt = '', $format = '' ) {
		if ( '' == $dt ) {
			$dt = get_the_time( 'U' );
		}
		if ( date( 'U' ) - $dt > intval( woohoo_get_theme_option( 'time_diff_before' ) ) * 24 * 3600 ) {
			$dt = date_i18n( '' == $format ? get_option( 'date_format' ) : $format, $dt );
		} else {
			// Translators: Add the human-friendly date difference
			$dt = sprintf( esc_html__( '%s ago', 'woohoo' ), human_time_diff( $dt, current_time( 'timestamp' ) ) );
		}
		return $dt;
	}
}
...
if ( ! function_exists( 'woohoo_lazy_load_off' ) ) {
	/**
	 * Disable a lazy load behaviour for rest images on the current page
	 */
	function woohoo_lazy_load_off() {
		if ( ! woohoo_lazy_load_is_off() ) {
			add_filter( 'wp_lazy_loading_enabled', 'woohoo_lazy_load_disabled' );
		}
	}
}

if ( ! function_exists( 'woohoo_lazy_load_disabled' ) ) {
	/**
	 ...
	 */
	function woohoo_lazy_load_disabled() {
		return false;
	}
}

if ( ! function_exists( 'woohoo_lazy_load_on' ) ) {
	/**
	 * Enable a lazy load behaviour for rest images on the current page
	 */
	function woohoo_lazy_load_on() {
		if ( woohoo_lazy_load_is_off() ) {
			remove_action( 'wp_lazy_loading_enabled', 'woohoo_lazy_load_disabled' );	// Is equal to rf
		}
	}
}

if ( ! function_exists( 'woohoo_lazy_load_is_off' ) ) {
	/**
	 * Return a current state of the lazy load behaviour
	 *
	 * @return bool  A current lazy load state
	 */
	function woohoo_lazy_load_is_off() {
		return has_filter( 'wp_lazy_loading_enabled', 'woohoo_lazy_load_disabled' );
	}
}
...
if ( ! function_exists( 'woohoo_get_markup_schema' ) ) {
	/**
	 * Return a structured data markup schema for the current page.
	 *
	 * @return string  A schema of the current page.
	 */
	function woohoo_get_markup_schema() {
		if ( woohoo_is_single() ) {                                        // Is single post
			$type = 'Article';
...
if ( ! function_exists( 'woohoo_get_privacy_text' ) ) {
	/**
	 * Return a text for the Privacy Policy checkbox.
	 *
	 * @return string  A label text.
	 */
	function woohoo_get_privacy_text() {
		$page         = get_option( 'wp_page_for_privacy_policy' );
		$privacy_text = woohoo_get_theme_option( 'privacy_text' );
		return apply_filters(
			'woohoo_filter_privacy_text',
			wp_kses(
				$privacy_text
				. ( ! empty( $page ) && ! empty( $privacy_text )
					// Translators: Add url to the Privacy Policy page
					? ' ' . sprintf( __( 'For further details on handling user data, see our %s.', 'woohoo' ),
						'<a href="' . esc_url( get_permalink( $page ) ) . '" target="_blank">'
						. __( 'Privacy Policy', 'woohoo' )
						. '</a>' )
					: ''
					),
				'woohoo_kses_content'
...
if ( ! function_exists( 'woohoo_kses_allowed_html' ) ) {
	add_filter( 'wp_kses_allowed_html', 'woohoo_kses_allowed_html', 10, 2 );
	/**
	 * Return a theme-specific tags list with allowed attributes for using in the wp_kses() calls
	 * ( if $context is equal to 'woohoo_kses_content' or 'trx_addons_kses_content' ).
	 *
	 * Hooks:
	 *
	 * add_filter( 'wp_kses_allowed_html', 'woohoo_kses_allowed_html', 10, 2 );
	 ...
	 */
	function woohoo_kses_allowed_html( $tags, $context ) {
		if ( in_array( $context, array( 'woohoo_kses_content', 'trx_addons_kses_content' ) ) ) {
			$tags = array( 
...
if ( ! function_exists( 'woohoo_verify_nonce' ) ) {
	/**
	 ...
	 */
	function woohoo_verify_nonce( $nonce = 'nonce', $mask = '' ) {
		if ( empty( $mask ) ) {
			$mask = admin_url('admin-ajax.php');
		}
		if ( ! wp_verify_nonce( woohoo_get_value_gp( $nonce ), $mask ) ) {
			woohoo_forbidden();
		}
	}
}

if ( ! function_exists( 'woohoo_exit' ) ) {
	/**
	 ...
	 */
	function woohoo_exit( $message = '', $title = '', $code = 200 ) {
		wp_die( $message, $title, array( 'response' => $code, 'exit' => empty( $message ) && empty( $title ) ) );
	}
}

if ( ! function_exists( 'woohoo_forbidden' ) ) {
	/**
	 ...
	 * @param string $title    Optional. Title to output while exit.
	 */
	function woohoo_forbidden( $message = '', $title = '' ) {
		woohoo_exit( $message, $title, 403 );
	}
}

if ( ! function_exists( 'woohoo_ajax_response' ) ) {
	/**
	 * Send an ajax response and exit.
	 *
	 * @param mixed $response  A response to send to the client.
	 */
	function woohoo_ajax_response( $response ) {
...
if ( ! function_exists( 'woohoo_get_upgrade_url' ) ) {
	/**
	 ...
	 */
	function woohoo_get_upgrade_url( $params = array() ) {
		$theme_folder = get_template();
		$theme_slug   = apply_filters( 'woohoo_filter_original_theme_slug', $theme_folder );
		$default      = array(
							'action'     => '',
		...
		if ( ! empty( $params['action'] ) && ! in_array( $params['action'], array( 'info_skins' ) ) ) {
			$default['key']        = '';
			$default['src']        = woohoo_get_theme_pro_key();
			$default['theme_name'] = wp_get_theme( $theme_folder )->get( 'Name' );
			$default['domain']     = woohoo_remove_protocol_from_url( get_home_url(), true );
		...
		return woohoo_add_to_url( trailingslashit( woohoo_storage_get( 'theme_upgrade_url' ) ) . 'upgrade.php', $params );
	}
}

if ( ! function_exists( 'woohoo_get_upgrade_data' ) ) {
	/**
	 ...
	 */
	function woohoo_get_upgrade_data( $params = array() ) {
		$url = woohoo_get_upgrade_url( $params );
		$result = function_exists( 'trx_addons_fgc' ) ? trx_addons_fgc( $url ) : woohoo_fgc( $url );
		if ( is_serialized( $result ) ) {
			try {
				$result = woohoo_unserialize( $result );
			} catch ( Exception $e ) {
			}
		}
		if ( ! isset( $result['error'] ) || ! isset( $result['data'] ) ) {
			$result = array(
				'error' => esc_html__( 'Unrecognized server answer!', 'woohoo' ),
				'data'  => ''
			);
		}
		return $result;
...
if ( ! function_exists( 'woohoo_disallow_upload_archives' ) ) {
	/**
	 * Disallow WordPress to upload archives after the upgrade package received.
	 */
	function woohoo_disallow_upload_archives() {
