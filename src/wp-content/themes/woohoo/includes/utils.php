...
if ( ! function_exists( 'woohoo_array_get_first' ) ) {
	/**
	 ...
	 */
	function woohoo_array_get_first( &$arr, $key = true ) {
		foreach ( $arr as $k => $v ) {
			break;
		}
		return $key ? $k : $v;
...
if ( ! function_exists( 'woohoo_array_get_keys_by_value' ) ) {
	/**
	 ...
	 */
	function woohoo_array_get_keys_by_value( $arr, $value = 1 ) {
		if ( ! is_array( $arr ) ) {
...
if ( ! function_exists( 'woohoo_array_delete_by_value' ) ) {
	/**
	 ...
	 */
	function woohoo_array_delete_by_value( $arr, $value ) {
...
if ( ! function_exists( 'woohoo_array_from_list' ) ) {
	/**
	 ...
	 */
	function woohoo_array_from_list( $arr ) {
		$new = array();
		foreach ( $arr as $v ) {
			$new[ $v ] = $v;
		}
...
if ( ! function_exists( 'woohoo_array_slice' ) ) {
	/**
	 ...
	 */
	function woohoo_array_slice( $arr, $from, $to='' ) {
...
if ( ! function_exists( 'woohoo_array_merge' ) ) {
	/**
	 * Merge arrays and lists (preserve a numeric indexes).
	 *
	 * For example:
	 *
	 * woohoo_array_merge( [ 1, 2, 3, 4 ], [ 'one', 'two' ] )  // result: [ 'one', 'two', 3, 4 ]
	 *
	 * woohoo_array_merge( [ 'a' => 'A', 'b' => 'B' ], [ 'one', 'two', 'a' => 'AA' ] )  // result: [ 'a' => 'AA', 'b' => 'B', 0 => 'one', 1 => 'two' ]
	 *
	 ...
	 */
	function woohoo_array_merge( $a1, $a2 ) {
...
if ( ! function_exists( 'woohoo_array_insert_after' ) ) {
	/**
	 ...
	 */
	function woohoo_array_insert_after( &$haystack, $needle, $stuff ) {
		...
		for ( $i = 2; $i < func_num_args(); ++$i ) {
			...
			if ( is_array( $arg ) ) {
				if ( 2 == $i ) {
					...
					$new_array = woohoo_array_merge( $new_array, $arg );
				}
			} else {
				$new_array[] = $arg;
			}
		...
		$haystack = is_int( $needle )
						...
						: woohoo_array_merge( array_slice( $haystack, 0, $i, true ), $new_array, array_slice( $haystack, $i, null, true ) );

		return $i;
	}
}

if ( ! function_exists( 'woohoo_array_insert_before' ) ) {
	/**
	 * Inserts any number of scalars or arrays at the point in the haystack
	 ...
	 * @return int               The index at which $needle was found.
	 */
	function woohoo_array_insert_before( &$haystack, $needle, $stuff ) {
		if ( ! is_array( $haystack ) ) {
			return -1;
		}
...
if ( ! function_exists( 'woohoo_array_get_by_key' ) ) {
	/**
	 ...
	 */
	function woohoo_array_get_by_key( &$arr, $key, $value, $key2 = '', $default = '' ) {
		$rez = $default;
		foreach ( $arr as $v ) {
...
if ( ! function_exists( 'woohoo_generate_id' ) ) {
	/**
	 ...
	 */
	function woohoo_generate_id( $prefix = '' ) {
		return $prefix . str_replace( '.', '', mt_rand() );
	}
}

if ( ! function_exists( 'woohoo_get_tag' ) ) {
	/**
	 ...
	 */
	function woohoo_get_tag( $text, $tag_start, $tag_end = '' ) {
		$val       = '';
...
if ( ! function_exists( 'woohoo_get_tag_attrib' ) ) {
	/**
	 ...
	 */
	function woohoo_get_tag_attrib( $text, $tag, $attr ) {
		$val       = '';
...
if ( ! function_exists( 'woohoo_get_css_position_from_values' ) ) {
	/**
	 ...
	 */
	function woohoo_get_css_position_from_values( $top = '', $right = '', $bottom = '', $left = '', $width = '', $height = '' ) {
		if ( ! is_array( $top ) ) {
		...
		foreach ( $top as $k => $v ) {
			...
			if ( '' != $v ) {
				$output .= ( 'width' == $k ? 'width' : ( 'height' == $k ? 'height' : 'margin-' . esc_attr( $k ) ) ) . ':' . esc_attr( woohoo_prepare_css_value( $v ) ) . ( '!' == $imp ? ' !important' : '' ) . ';';
			}
		}
		return $output;
	}
...
if ( ! function_exists( 'woohoo_prepare_css_value' ) ) {
	/**
	 ...
	 */
	function woohoo_prepare_css_value( $val ) {
		if ( '' !== $val ) {
...
if ( ! function_exists( 'woohoo_parse_icons_classes' ) ) {
	/**
	 ...
	 */
	function woohoo_parse_icons_classes( $css ) {
		$rez = array();
		if ( ! file_exists( $css ) ) {
			return $rez;
		}
...
if ( ! function_exists( 'woohoo_get_column_class' ) ) {
	/**
	 ...
	 */
	function woohoo_get_column_class( $num, $all, $all_tablet = '', $all_mobile = '' ) {
...
if ( ! function_exists( 'woohoo_stripslashes' ) ) {
	/**
	 ...
	 */
	function woohoo_stripslashes( $val ) {
		static $magic = 0;
		if ( 0 === $magic ) {
...
if ( ! function_exists( 'woohoo_get_value_gp' ) ) {
	/**
	 ...
	 */
	function woohoo_get_value_gp( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
...
if ( ! function_exists( 'woohoo_get_value_gpc' ) ) {
	/**
	 ...
	 */
	function woohoo_get_value_gpc( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
			$rez = wp_unslash( $_POST[ $name ] );
...
if ( ! function_exists( 'woohoo_get_value_gps' ) ) {
	/**
	 ...
	 */
	function woohoo_get_value_gps( $name, $defa = '' ) {
		global $wp_session;
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
...
if ( ! function_exists( 'woohoo_get_session_value' ) ) {
	/**
	 ...
	 */
	function woohoo_get_session_value( $name, $defa = '' ) {
		global $wp_session;
		return isset( $wp_session[ $name ] ) ? $wp_session[ $name ] : $defa;
	}
}

if ( ! function_exists( 'woohoo_set_session_value' ) ) {
	/**
	 ...
	 */
	function woohoo_set_session_value( $name, $value ) {
		global $wp_session;
		$wp_session[ $name ] = $value;
	}
}

if ( ! function_exists( 'woohoo_del_session_value' ) ) {
	/**
	 * Delete a saved variable from the current session.
	 *
	 * @param string $name   A name to to delete from the session.
	 */
	function woohoo_del_session_value( $name ) {
		global $wp_session;
		unset( $wp_session[ $name ] );
	}
}

if ( ! function_exists( 'woohoo_get_cookie' ) ) {
	/**
	 ...
	 */
	function woohoo_get_cookie( $name, $defa = '' ) {
		return woohoo_stripslashes( isset( $_COOKIE[ $name ] ) ? $_COOKIE[ $name ] : $defa );
	}
}

// Set a cookie - wrapper for setcookie using WP constants.
if ( ! function_exists( 'woohoo_set_cookie' ) ) {
	/**
	 ...
	 */
	function woohoo_set_cookie( $name, $value, $expire = 0, $secure = false, $httponly = false  ) {
		if ( ! headers_sent() ) {
			if ( defined( 'PHP_VERSION_ID' ) && PHP_VERSION_ID >= 70300 ) {
				$rez = setcookie(
							$name,
							$value,
							apply_filters( 'woohoo_filter_cookie_options', array(
								'expires'  => $expire,
								'path'     => defined( 'COOKIEPATH' ) && ! empty( COOKIEPATH ) ? COOKIEPATH : '/',
								'domain'   => defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//woohoo_get_domain_from_url( get_home_url() ),
								'secure'   => $secure,
								'httponly' => $httponly,
				...
				$rez = setcookie(
							...
							defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//woohoo_get_domain_from_url( get_home_url() ),
							$secure,
							$httponly
						);
			}
			...
			trigger_error( sprintf( __( '%1$s cookie cannot be set - headers already sent by %2$s on line %3$s', 'woohoo' ), $name, $file, $line ), E_USER_NOTICE ); // @codingStandardsIgnoreLine
		}
	}
}
...
if ( ! function_exists( 'woohoo_hex2rgb' ) ) {
	/**
...
if ( ! function_exists( 'woohoo_hex2rgba' ) ) {
	/**
	 ...
	 */
	function woohoo_hex2rgba( $hex, $alpha ) {
		$rgb = woohoo_hex2rgb( $hex );
		return 'rgba(' . intval( $rgb['r'] ) . ',' . intval( $rgb['g'] ) . ',' . intval( $rgb['b'] ) . ',' . floatval( $alpha ) . ')';
	}
}

if ( ! function_exists( 'woohoo_hex2hsb' ) ) {
	/**
	 ...
	 */
	function woohoo_hex2hsb( $hex, $h = 0, $s = 0, $b = 0 ) {
		$hsb      = woohoo_rgb2hsb( woohoo_hex2rgb( $hex ) );
...
if ( ! function_exists( 'woohoo_rgb2hsb' ) ) {
	/**
...
if ( ! function_exists( 'woohoo_hsb2rgb' ) ) {
	/**
...
if ( ! function_exists( 'woohoo_rgb2hex' ) ) {
	/**
...
if ( ! function_exists( 'woohoo_hsb2hex' ) ) {
	/**
	 ...
	 * @return string     A string with a hex color presentation in the format '#RRGGBB'.
	 */
	function woohoo_hsb2hex( $hsb ) {
		return woohoo_rgb2hex( woohoo_hsb2rgb( $hsb ) );
	}
}
...
//
if ( ! function_exists( 'woohoo_date_to_sql' ) ) {
	/**
	 ...
	 */
	function woohoo_date_to_sql($str) {
		if (trim($str)=='') return '';
		$str = strtr(trim($str),'/\.,','----');
...
if ( ! function_exists( 'woohoo_format_price' ) ) {
	/**
	 ...
	 */
	function woohoo_format_price( $price ) {
...
if ( ! function_exists( 'woohoo_num2size' ) ) {
	/**
	 ...
	 */
	function woohoo_num2size( $num ) {
		return $num > 1000 ? round( $num/1000, 0 ) . 'K' : $num;
	}
}

if ( ! function_exists( 'woohoo_size2num' ) ) {
	/**
	 ...
	 */
	function woohoo_size2num( $size, $full = false ) {
...
//
if ( ! function_exists( 'woohoo_parse_num' ) ) {
	/**
...
if ( ! function_exists( 'woohoo_prepare_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function woohoo_prepare_macros( $str ) {
		return str_replace(
			array( '{{', '}}', '((', '))', '||' ),
			array( '<i>', '</i>', '<b>', '</b>', '<br>' ),
			$str
...
if ( ! function_exists( 'woohoo_remove_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function woohoo_remove_macros( $str ) {
		return str_replace(
			array( '{{', '}}', '((', '))', '||' ),
			array( '', '', '', '', ' ' ),
			$str
...
if ( ! function_exists( 'woohoo_is_on' ) ) {
	/**
...
if ( ! function_exists( 'woohoo_is_off' ) ) {
	/**
...
if ( ! function_exists( 'woohoo_is_inherit' ) ) {
	/**
	 ...
	 */
	function woohoo_is_inherit( $prm ) {
		return ! is_array( $prm ) && in_array( strtolower( $prm ), array( 'inherit' ) );
	}
}

if ( ! function_exists( 'woohoo_strshort' ) ) {
	/**
	 ...
	 */
	function woohoo_strshort( $str, $maxlength, $add = '&hellip;' ) {
		if ( 0 >= $maxlength ) {
			return '';
		}
		$str = woohoo_strip_tags( $str );
...
if ( ! function_exists( 'woohoo_strwords' ) ) {
	/**
	 ...
	 * @return string            A truncated string.
	 */
	function woohoo_strwords( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		$words = explode( ' ', $str );
...
if ( ! function_exists( 'woohoo_strip_tags' ) ) {
	/**
	 ...
	 */
	function woohoo_strip_tags( $str ) {
...
if ( ! function_exists( 'woohoo_excerpt' ) ) {
	/**
	 ...
	 */
	function woohoo_excerpt( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		return woohoo_strwords( woohoo_strip_tags( $str ), $maxlength, $add );
	}
}

if ( ! function_exists( 'woohoo_unserialize_recover' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function woohoo_unserialize_recover( $str ) {
...
if ( ! function_exists( 'woohoo_unserialize' ) ) {
	/**
	 ...
	 */
	function woohoo_unserialize( $str ) {
		if ( ! empty( $str ) && is_serialized( $str ) ) {
			...
			if ( true || ! preg_match( '/O:[0-9]+:"([^"]*)":[0-9]+:{/', $str, $matches ) || empty( $matches[1] ) || class_exists( $matches[1] ) ) {
				try {
					...
					if ( false === $data ) {
						$data = @unserialize( woohoo_unserialize_recover( $str ) );
					}
					// Attempt 4: try unserialize original string without CR symbol '\r' with modified character counters
					if ( false === $data ) {
						$data = @unserialize( woohoo_unserialize_recover( $str2 ) );
					}
				} catch ( Exception $e ) {
...
if ( ! function_exists( 'woohoo_str_replace' ) ) {
	/**
	 ...
	 */
	function woohoo_str_replace( $from, $to, $str ) {
		if ( is_array( $str ) ) {
			foreach ( $str as $k => $v ) {
				$str[ $k ] = woohoo_str_replace( $from, $to, $v );
			}
			...
			if ( is_serialized( $str ) ) {
				$str = serialize( woohoo_str_replace( $from, $to, woohoo_unserialize( $str ) ) );
			} else {
...
if ( ! function_exists( 'woohoo_str_replace_once' ) ) {
	/**
	 ...
	 * @return string             A processed string.
	 */
	function woohoo_str_replace_once( $from, $to, $str ) {
		$rez = '';
		if ( ! is_array( $from ) ) {
			$from = array( $from );
		}
...
if ( ! function_exists( 'woohoo_tags_count' ) ) {
	/**
	 ...
	 */
	function woohoo_tags_count( $str, $tag ) {
		$cnt = 0;
...
if ( ! function_exists( 'woohoo_getimagesize' ) ) {
	/**
	 ...
	 */
	function woohoo_getimagesize( $url, $echo = false ) {
		$img_size = false;
		$img_path = woohoo_is_url( $url ) ? woohoo_url_to_local_path( $url ) : $url;
...
if ( ! function_exists( 'woohoo_clear_thumb_size' ) ) {
	/**
	 ...
	 */
	function woohoo_clear_thumb_size( $url, $remove_protocol = true ) {
		if ( empty( $url ) ) return '';
		$pi = pathinfo( $url );
		if ( $remove_protocol ) {
			$pi['dirname'] = woohoo_remove_protocol_from_url( $pi['dirname'], false );
		}
...
if ( ! function_exists( 'woohoo_add_thumb_size' ) ) {
	/**
	 ...
	 */
	function woohoo_add_thumb_size( $url, $thumb_size, $check_exists = true ) {
		...
		$new_url = woohoo_remove_protocol_from_url( ( ! empty( $pi['dirname'] ) ? $pi['dirname'] . '/' : '' ) . $pi['filename'] . ( ! empty( $pi['extension'] ) ? '.' . $pi['extension'] : '' ) );

		// Check exists
		if ( $check_exists ) {
			$uploads_info = wp_upload_dir();
			$uploads_url = woohoo_remove_protocol_from_url( $uploads_info['baseurl'] );
			...
			if ( strpos( $new_url, $uploads_url ) !== false ) {
				if ( ! file_exists( str_replace( $uploads_url, $uploads_dir, $new_url ) ) ) {
					$new_url = woohoo_remove_protocol_from_url( $url );
				}
			} else {
				$new_url = woohoo_remove_protocol_from_url( $url );
			}
...
if ( ! function_exists( 'woohoo_get_thumb_size' ) ) {
	 ...
	 */
	function woohoo_get_thumb_size( $ts ) {
		$thumb_sizes = woohoo_storage_get( 'theme_thumbs' );
		if ( ! empty( $thumb_sizes[ "woohoo-thumb-{$ts}" ] ) ) {
			$ts = 'woohoo-thumb-' . $ts;
		}
		$retina = woohoo_get_retina_multiplier() > 1 ? '-@retina' : '';
		$is_external = apply_filters(
							'woohoo_filter_is_external_thumb_size',
							in_array( $ts,
										apply_filters(
											'woohoo_filter_external_thumb_sizes',
		...
		$is_internal = strpos( $ts, 'woohoo-thumb-' ) === 0 || strpos( $ts, 'trx_addons-thumb-' ) === 0;
		return apply_filters(
					'woohoo_filter_get_thumb_size',
					( $is_external || $is_internal ? '' : 'woohoo-thumb-' )
					. $ts
					. ( $is_internal ? $retina : '' )
				);
	}
...
if ( ! function_exists( 'woohoo_get_closest_thumb_size' ) ) {
	 ...
	 */
	function woohoo_get_closest_thumb_size( $old_size, $dim, $prefix = '' ) {
...
if ( ! function_exists( 'woohoo_get_attachment_url' ) ) {
	/**
	 ...
	 * @return string
	 */
	function woohoo_get_attachment_url( $image_id, $size = 'full' ) {
		...
		if ( is_numeric( $image_id ) && (int) $image_id > 0 ) {
			...
			$image_id = woohoo_add_thumb_size( $image_id, $size );
		}
		return $image_id;
	}
}

if ( ! function_exists( 'woohoo_get_post_image' ) ) {
	/**
	 ...
	 */
	function woohoo_get_post_image( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		if ( empty( $post_text ) ) {
...
if ( ! function_exists( 'woohoo_get_post_audio' ) ) {
	/**
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function woohoo_get_post_audio( $post_text = '', $src = true ) {
		global $post;
		$img = woohoo_get_post_audio_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['url'] ) ) {
				$img = $img['url'];
			} else if ( ! empty( $img['embed'] ) ) {
				$img = woohoo_get_post_iframe( $img['embed'], true );
			} else {
				$img = '';
			}
		}
		if ( empty( $img ) ) {
			...
			if ( $src ) {
				...
				$img = woohoo_get_tag( $post_text, '<audio', '</audio>' );
				if ( empty( $img ) ) {
					$img = do_shortcode( woohoo_get_tag( $post_text, '[audio', '[/audio]' ) );
				}
				if ( empty( $img ) ) {
					$img = woohoo_get_tag_attrib( $post_text, '[trx_widget_audio]', 'url' );
...
if ( ! function_exists( 'woohoo_get_post_audio_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function woohoo_get_post_audio_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_audio_list' ) ? trx_addons_get_post_audio_list( $args ) : array();
	}
}

if ( ! function_exists( 'woohoo_get_post_audio_list_first' ) ) {
	/**
...
if ( ! function_exists( 'woohoo_get_post_video' ) ) {
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function woohoo_get_post_video( $post_text = '', $src = true ) {
		global $post;
		$img = woohoo_get_post_video_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['video_url'] ) ) {
				$img = $img['video_url'];
			} else if ( ! empty( $img['video_embed'] ) ) {
				$img = woohoo_get_post_iframe( $img['video_embed'], true );
			} else {
				$img = '';
			}
		}
		...
		return apply_filters( 'woohoo_filter_get_post_video', $img );
	}
}

if ( ! function_exists( 'woohoo_get_post_video_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function woohoo_get_post_video_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_video_list' ) ? trx_addons_get_post_video_list( $args ) : array();
	}
}

if ( ! function_exists( 'woohoo_get_post_video_list_first' ) ) {
	/**
...
if ( ! function_exists( 'woohoo_get_post_iframe' ) ) {
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function woohoo_get_post_iframe( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		$tag = woohoo_get_embed_video_tag_name();
		if ( empty( $post_text ) ) {
			$post_text = $post->post_content;
		}
		if ( $src ) {
			...
			$img = woohoo_get_tag( $post_text, '<' . esc_html( $tag ), '</' . esc_html( $tag ) . '>' );
		}
		return apply_filters( 'woohoo_filter_get_post_iframe', $img );
	}
}

if ( ! function_exists( 'woohoo_get_embed_video' ) ) {
	/**
	...
	function woohoo_get_embed_video( $video, $use_wp_embed = false, $args = array() ) {
		...
		if ( $use_wp_embed && is_object( $wp_embed ) ) {
			...
			if ( strpos( $video, 'youtu.be/' ) !== false || strpos( $video, 'youtube.com/' ) !== false ) {
				...
				$video = woohoo_add_to_url( $video, woohoo_get_embed_video_parameters() );
			}
			...
			$ratio  = apply_filters( 'woohoo_filter_video_ratio', $is_tiktok ? '9:21' : '16:9' );
			$parts  = explode( ':', $ratio );
			$width  = $is_tiktok ? apply_filters( 'woohoo_filter_video_width', 325, 'tiktok' ) : woohoo_get_content_width();
			$height = round( $width / $parts[0] * $parts[1] );
			$dim    = apply_filters( 'woohoo_filter_video_dimensions', array(
									'width' => $width,
									'height' => $height
									), $ratio );
...
if ( ! function_exists( 'woohoo_get_embed_video_tag_name' ) ) {
	/**
	 * Return a name of the tag-container to embed video.
	 *
	 * @return string  A tag name to embed video.
	 */
	function woohoo_get_embed_video_tag_name() {
...
if ( ! function_exists( 'woohoo_make_video_autoplay' ) ) {
	/**
	 ...
	 */
	function woohoo_make_video_autoplay( $video, $muted = false ) {
					...
					if ( ! empty( $matches[2] ) ) {
						...
						$matches[2] = woohoo_add_to_url(
							...
							array_merge(
								...
								$muted
									? array_merge(
										woohoo_get_embed_video_parameters(),
										array(
											'muted' => 1,
											'controls' => 0,
											'background' => 1,
...
if ( ! function_exists( 'woohoo_is_from_uploads' ) ) {
	/**
	 ...
	 */
	function woohoo_is_from_uploads( $url ) {
		$url          = woohoo_remove_protocol_from_url( $url );
		$parts        = explode( '?', $url );
		$url          = $parts[0];
		$uploads_info = wp_upload_dir();
		$uploads_url  = woohoo_remove_protocol_from_url( $uploads_info['baseurl'] );
...
if ( ! function_exists( 'woohoo_is_youtube_url' ) ) {
	/**
	 ...
	 */
	function woohoo_is_youtube_url( $url ) {
		return strpos( $url, 'youtu.be' ) !== false || strpos( $url, 'youtube.com' ) !== false;
	}
}

if ( ! function_exists( 'woohoo_is_vimeo_url' ) ) {
	/**
	 ...
	 */
	function woohoo_is_vimeo_url( $url ) {
...
if ( ! function_exists( 'woohoo_init_filesystem' ) ) {
	add_action( 'after_setup_theme', 'woohoo_init_filesystem', 0 );
	/**
	 ...
	 */
	function woohoo_init_filesystem( $force = false ) {
...
if ( ! function_exists( 'woohoo_fpc' ) ) {
	 ...
	 */
	function woohoo_fpc( $file, $data, $flag = 0 ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fpc' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( woohoo_is_on( woohoo_get_theme_option( 'debug_mode' ) ) ) {
						...
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Put contents to the file "%s" failed', 'woohoo' ), $file ) );
					}
				}
			}
		}
	...
	function woohoo_fgc( $file ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fgc' ) ) {
				...
				global $wp_filesystem;
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					$file = substr( $file, 0, 2 ) == '//' ? woohoo_add_protocol( $file ) : str_replace( ABSPATH, $wp_filesystem->abspath(), $file );
					return $wp_filesystem->get_contents( $file );
				} else {
					if ( woohoo_is_on( woohoo_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get contents from the file "%s" failed', 'woohoo' ), $file ) );
					}
				}
			}
		}
		...
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fga' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( woohoo_is_on( woohoo_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get rows from the file "%s" failed', 'woohoo' ), $file ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'woohoo_mkdir' ) ) {
	/**
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( ! $wp_filesystem->is_dir( $path ) ) {
						...
						if ( ! wp_mkdir_p( $path ) ) {
							if ( woohoo_is_on( woohoo_get_theme_option( 'debug_mode' ) ) ) {
								// Translators: Add the file name to the message
								throw new Exception( sprintf( esc_html__( 'Create a folder "%s" failed', 'woohoo' ), $path ) );
							}
						} else {
							return true;
						}
					...
					if ( woohoo_is_on( woohoo_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Create a folder "%s" failed', 'woohoo' ), $path ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'woohoo_unlink' ) ) {
	/**
	 ...
	 * @throws Exception        Throw an Exception if the object $wp_filesystem is not initialized yet or if error occurs.
	 */
	function woohoo_unlink( $path, $recursive = true, $type = 'd' ) {
		if ( ! empty( $path ) ) {
			if ( function_exists( 'trx_addons_unlink' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( woohoo_is_on( woohoo_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Delete a file/folder "%s" failed', 'woohoo' ), $path ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'woohoo_copy' ) ) {
	/**
	...
	function woohoo_copy( $src, $dst ) {
		...
		if ( ! empty( $src ) && ! empty( $dst ) ) {
			if ( function_exists( 'trx_addons_copy' ) ) {
				...
				$src = woohoo_prepare_path( $src );
				$dst = woohoo_prepare_path( $dst );
...
if ( ! function_exists( 'woohoo_unzip_file' ) ) {
	/**
...
if ( ! function_exists( 'woohoo_esc' ) ) {
	 ...
	 */
	function woohoo_esc( $name ) {
		return str_replace( array( '\\', '~', '$', ':', ';', '+', '>', '<', '|', '"', "'", '`', "\xFF", "\x0A", "\x0D", '*', '?', '^' ), '/', trim( $name ) );
	}
}

if ( ! function_exists( 'woohoo_prepare_path' ) ) {
	/**
...
if ( ! function_exists( 'woohoo_check_min_file' ) ) {
	 ...
	 */
	function woohoo_check_min_file( $file, $dir = '' ) {
		if ( empty( $dir ) ) {
			$dir = dirname( $file );
		}
		if ( substr( $file, -3 ) == '.js' ) {
			if ( substr( $file, -7 ) != '.min.js' && woohoo_is_off( woohoo_get_theme_option( 'debug_mode' ) ) ) {
			...
			if ( substr( $file, -8 ) != '.min.css' && woohoo_is_off( woohoo_get_theme_option( 'debug_mode' ) ) ) {
				$dir      = trailingslashit( $dir );
...
if ( ! function_exists( 'woohoo_get_file_dir' ) ) {
	/**
	 ...
	 */
	function woohoo_get_file_dir( $file, $return_url = false ) {
		// Use new WordPress functions (if present)
		if ( function_exists( 'get_theme_file_path' ) && ! WOOHOO_ALLOW_SKINS && ! woohoo_get_theme_setting( 'check_min_version', false ) ) {
		...
		// Otherwise (on WordPress older then 4.7.0) or theme use .min versions of .js and .css or theme use skins
			...
			$dir       = '';
			$theme_dir = apply_filters( 'woohoo_filter_get_theme_file_dir', '', $file, $return_url );
			if ( '' != $theme_dir ) {
				$dir = $theme_dir;
			} elseif ( WOOHOO_CHILD_DIR != WOOHOO_THEME_DIR && file_exists( WOOHOO_CHILD_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? WOOHOO_CHILD_URL : WOOHOO_CHILD_DIR ) . woohoo_check_min_file( $file, WOOHOO_CHILD_DIR );
			} elseif ( file_exists( WOOHOO_THEME_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? WOOHOO_THEME_URL : WOOHOO_THEME_DIR ) . woohoo_check_min_file( $file, WOOHOO_THEME_DIR );
			}
		}
		return $dir;
	}
...
if ( ! function_exists( 'woohoo_get_file_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a file is not exists - return an empty string.
	 */
	function woohoo_get_file_url( $file ) {
		return woohoo_get_file_dir( $file, true );
	}
}

if ( ! function_exists( 'woohoo_get_file_ext' ) ) {
	/**
	 ...
	 */
	function woohoo_get_file_ext( $file ) {
		$ext = pathinfo( $file, PATHINFO_EXTENSION );
		return empty( $ext ) ? '' : $ext;
	}
}

if ( ! function_exists( 'woohoo_get_file_name' ) ) {
	/**
	 ...
	 */
	function woohoo_get_file_name( $file, $without_ext = true ) {
		$parts = pathinfo( $file );
		return ! empty( $parts['filename'] ) && $without_ext ? $parts['filename'] : $parts['basename'];
	}
}

if ( ! function_exists( 'woohoo_get_folder_dir' ) ) {
	/**
	 ...
	 */
	function woohoo_get_folder_dir( $folder, $return_url = false ) {
		if ( '/' == $folder[0] ) {
			$folder = substr( $folder, 1 );
		}
		$dir       = '';
		$theme_dir = apply_filters( 'woohoo_filter_get_theme_folder_dir', '', $folder, $return_url );
		if ( '' != $theme_dir ) {
			$dir = $theme_dir;
		} elseif ( WOOHOO_CHILD_DIR != WOOHOO_THEME_DIR && is_dir( WOOHOO_CHILD_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? WOOHOO_CHILD_URL : WOOHOO_CHILD_DIR ) . ( $folder );
		} elseif ( is_dir( WOOHOO_THEME_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? WOOHOO_THEME_URL : WOOHOO_THEME_DIR ) . ( $folder );
		}
		return apply_filters( 'woohoo_filter_get_folder_dir', $dir, $folder, $return_url );
	}
}

if ( ! function_exists( 'woohoo_get_folder_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a folder is not exists - return an empty string.
	 */
	function woohoo_get_folder_url( $folder ) {
		return woohoo_get_folder_dir( $folder, true );
	}
}

if ( ! function_exists( 'woohoo_merge_js' ) ) {
	/**
	 ...
	 */
	function woohoo_merge_js( $to, $list ) {
		$s = '';
		foreach ( $list as $f ) {
			$s .= woohoo_fgc( woohoo_get_file_dir( $f ) );
		}
		if ( '' != $s ) {
			$file_dir = woohoo_get_file_dir( $to );
			if ( empty( $file_dir ) && strpos( $to, '-full.js' ) !== false ) {
				$file_dir = woohoo_get_file_dir( str_replace( '-full.js', '.js', $to ) );
				if ( ! empty( $file_dir ) ) {
			...
			woohoo_fpc( $file_dir,
				'/* '
				. strip_tags( __( "ATTENTION! This file was generated automatically! Don't change it!!!", 'woohoo' ) )
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'woohoo_filter_js_output', apply_filters( 'woohoo_filter_prepare_js', $s, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'woohoo_merge_css' ) ) {
	/**
	 ...
	 */
	function woohoo_merge_css( $to, $list, $need_responsive = false ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'woohoo_filter_responsive_sizes', woohoo_storage_get( 'responsive' ) );
		}
		$sizes  = array();
		$output = '';
		foreach ( $list as $f ) {
			$fdir = woohoo_get_file_dir( $f );
			if ( '' != $fdir ) {
				$css = woohoo_fgc( $fdir );
				if ( $need_responsive ) {
					$pos = 0;
					while( false !== $pos ) {
						...
						if ( false !== $pos ) {
							...
							$sizes[ $media ] .= "\n\n" . apply_filters( 'woohoo_filter_merge_css', substr( $css, $pos_lbrace + 1, $pos_rbrace - $pos_lbrace - 1 ) );
							$css = substr( $css, $pos_rbrace + 1);
						}
					}
				} else {
					$output .= "\n\n" . apply_filters( 'woohoo_filter_merge_css', $css );
				}
			}
		}
		if ( $need_responsive ) {
			foreach ( $responsive as $k => $v ) {
				...
				if ( ! empty( $sizes[ $media ] ) ) {
					$output .= "\n\n"
							...
							. strip_tags( sprintf( __( '/* SASS Suffix: --%s */', 'woohoo' ), $k ) )
							. "\n"
							. "@media {$media} {\n"
								. $sizes[ $media ]
							. "\n}\n";
			...
			if ( count( $sizes ) > 0 ) {
				$output .= "\n\n"
						. strip_tags( __( '/* Unknown Suffixes: */', 'woohoo' ) );
		...
		if ( '' != $output ) {
			...
			if ( empty( $file_dir ) && strpos( $to, '-full.css' ) !== false ) {
				$file_dir = woohoo_get_file_dir( str_replace( '-full.css', '.css', $to ) );
				if ( ! empty( $file_dir ) ) {
			...
			woohoo_fpc( $file_dir,
				...
				. strip_tags( __("ATTENTION! This file was generated automatically! Don't change it!!!", 'woohoo') ) 
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'woohoo_filter_css_output', apply_filters( 'woohoo_filter_prepare_css', $output, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'woohoo_filter_merge_css' ) ) {
	add_filter( 'woohoo_filter_merge_css', 'woohoo_filter_merge_css' );
	/**
	 ...
	 */
	function woohoo_filter_merge_css( $css ) {
		return str_replace( '../../../../', '../../../', $css);
	}
}

if ( ! function_exists( 'woohoo_merge_sass' ) ) {
	/**
	 ...
	 */
	function woohoo_merge_sass( $to, $list, $need_responsive = false, $root = '../' ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'woohoo_filter_responsive_sizes', woohoo_storage_get( 'responsive' ) );
		}
		$sass                = array(
		...
		foreach ( $list as $f ) {
			...
			if ( $add ) {
				$sass['import'] .= apply_filters( 'woohoo_filter_sass_import', "{$sass_import} \"{$root}{$f}\";\n", $f );
				$save            = true;
			}
		}
		if ( $save ) {
			...
			woohoo_fpc( woohoo_get_file_dir( $to ), apply_filters( 'woohoo_filter_sass_output', $output, $to ) );
		}
	}
}
...
if ( ! function_exists( 'woohoo_get_domain_from_url' ) ) {
	/**
	 ...
	 */
	function woohoo_get_domain_from_url( $url ) {
		$pos = strpos( $url, '//' );
		if ( $pos !== false ) {
...
if ( ! function_exists( 'woohoo_add_protocol' ) ) {
	/**
	 ...
	 */
	function woohoo_add_protocol( $url ) {
		return substr( $url, 0, 2 ) == '//' ? woohoo_get_protocol() . ':' . $url : $url;
	}
}

if ( ! function_exists( 'woohoo_remove_protocol_from_url' ) ) {
	/**
	 ...
	 * @return string         A processed string.
	 */
	function woohoo_remove_protocol_from_url( $url, $complete = true ) {
		return preg_replace( '/(http[s]?:)?' . ( $complete ? '\\/\\/' : '' ) . '/', '', $url );
	}
}

if ( ! function_exists( 'woohoo_add_to_url' ) ) {
	/**
	 ...
	 */
	function woohoo_add_to_url( $url, $prm ) {
		if ( is_array( $prm ) && count( $prm ) > 0 ) {
			...
			$params = woohoo_array_merge( $params, $prm );
			$url = $parts[0];
			$separator = '?';
...
if ( ! function_exists( 'woohoo_is_url' ) ) {
	/**
	 ...
	 */
	function woohoo_is_url( $url ) {
		return strpos( $url, '//' ) === 0 || strpos( $url, '://' ) !== false;
	}
}

if ( ! function_exists( 'woohoo_is_external_url' ) ) {
	/**
	 ...
	 */
	function woohoo_is_external_url( $url ) {
		return woohoo_is_url( $url ) && strpos( $url, woohoo_remove_protocol_from_url( get_home_url(), true ) ) === false;
	}
}

if ( ! function_exists( 'woohoo_url_to_local_path' ) ) {
	/**
	 ...
	 */
	function woohoo_url_to_local_path( $url ) {
		$path = '';
		...
		$locations = array(
			'uploads' => array(
				...
				'url' => woohoo_remove_protocol_from_url( $upload_info['baseurl'] )
				),
			'child' => array(
				'dir' => WOOHOO_CHILD_DIR,
				'url' => woohoo_remove_protocol_from_url( WOOHOO_CHILD_URL )
				),
			'theme' => array(
				'dir' => WOOHOO_THEME_DIR,
				'url' => woohoo_remove_protocol_from_url( WOOHOO_THEME_URL )
				)
			);
