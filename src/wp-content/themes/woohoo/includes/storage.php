...
if ( ! function_exists( 'woohoo_storage_get' ) ) {
	/**
	 ...
	 */
	function woohoo_storage_get( $var_name, $default = '' ) {
		global $WOOHOO_STORAGE;
		return isset( $WOOHOO_STORAGE[ $var_name ] ) ? $WOOHOO_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'woohoo_storage_set' ) ) {
	/**
	 ...
	 */
	function woohoo_storage_set( $var_name, $value ) {
		global $WOOHOO_STORAGE;
		$WOOHOO_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'woohoo_storage_empty' ) ) {
	/**
	 ...
	 */
	function woohoo_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $WOOHOO_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $WOOHOO_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $WOOHOO_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $WOOHOO_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'woohoo_storage_isset' ) ) {
	/**
	 ...
	 */
	function woohoo_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $WOOHOO_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $WOOHOO_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $WOOHOO_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $WOOHOO_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'woohoo_storage_unset' ) ) {
	/**
	 ...
	 */
	function woohoo_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $WOOHOO_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $WOOHOO_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $WOOHOO_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $WOOHOO_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'woohoo_storage_inc' ) ) {
	/**
	 ...
	 */
	function woohoo_storage_inc( $var_name, $value = 1 ) {
		global $WOOHOO_STORAGE;
		if ( empty( $WOOHOO_STORAGE[ $var_name ] ) ) {
			$WOOHOO_STORAGE[ $var_name ] = 0;
		}
		$WOOHOO_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'woohoo_storage_concat' ) ) {
	/**
	 ...
	 */
	function woohoo_storage_concat( $var_name, $value ) {
		global $WOOHOO_STORAGE;
		if ( empty( $WOOHOO_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'woohoo_storage_get_array' ) ) {
	/**
	 ...
	 */
	function woohoo_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $WOOHOO_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $WOOHOO_STORAGE[ $var_name ][ $key ] ) ? $WOOHOO_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $WOOHOO_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $WOOHOO_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'woohoo_storage_set_array' ) ) {
	/**
	 ...
	 */
	function woohoo_storage_set_array( $var_name, $key, $value ) {
		global $WOOHOO_STORAGE;
		if ( ! isset( $WOOHOO_STORAGE[ $var_name ] ) ) {
			$WOOHOO_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$WOOHOO_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'woohoo_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function woohoo_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $WOOHOO_STORAGE;
		if ( ! isset( $WOOHOO_STORAGE[ $var_name ] ) ) {
			$WOOHOO_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $WOOHOO_STORAGE[ $var_name ][ $key ] ) ) {
			$WOOHOO_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$WOOHOO_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$WOOHOO_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'woohoo_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function woohoo_storage_merge_array( $var_name, $key, $value ) {
		global $WOOHOO_STORAGE;
		if ( ! isset( $WOOHOO_STORAGE[ $var_name ] ) ) {
			$WOOHOO_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$WOOHOO_STORAGE[ $var_name ] = array_merge( $WOOHOO_STORAGE[ $var_name ], $value );
		} else {
			$WOOHOO_STORAGE[ $var_name ][ $key ] = array_merge( $WOOHOO_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'woohoo_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function woohoo_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $WOOHOO_STORAGE;
		if ( ! isset( $WOOHOO_STORAGE[ $var_name ] ) ) {
			$WOOHOO_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			woohoo_array_insert_after( $WOOHOO_STORAGE[ $var_name ], $after, $key );
		} else {
			woohoo_array_insert_after( $WOOHOO_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'woohoo_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function woohoo_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $WOOHOO_STORAGE;
		if ( ! isset( $WOOHOO_STORAGE[ $var_name ] ) ) {
			$WOOHOO_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			woohoo_array_insert_before( $WOOHOO_STORAGE[ $var_name ], $before, $key );
		} else {
			woohoo_array_insert_before( $WOOHOO_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'woohoo_storage_push_array' ) ) {
	/**
	 ...
	 */
	function woohoo_storage_push_array( $var_name, $key, $value ) {
		global $WOOHOO_STORAGE;
		if ( ! isset( $WOOHOO_STORAGE[ $var_name ] ) ) {
			$WOOHOO_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $WOOHOO_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $WOOHOO_STORAGE[ $var_name ][ $key ] ) ) {
				$WOOHOO_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $WOOHOO_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'woohoo_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function woohoo_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $WOOHOO_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $WOOHOO_STORAGE[ $var_name ] ) && is_array( $WOOHOO_STORAGE[ $var_name ] ) && count( $WOOHOO_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $WOOHOO_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $WOOHOO_STORAGE[ $var_name ][ $key ] ) && is_array( $WOOHOO_STORAGE[ $var_name ][ $key ] ) && count( $WOOHOO_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $WOOHOO_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'woohoo_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function woohoo_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $WOOHOO_STORAGE;
		if ( ! isset( $WOOHOO_STORAGE[ $var_name ] ) ) {
			$WOOHOO_STORAGE[ $var_name ] = array();
		}
		if ( empty( $WOOHOO_STORAGE[ $var_name ][ $key ] ) ) {
			$WOOHOO_STORAGE[ $var_name ][ $key ] = 0;
		}
		$WOOHOO_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'woohoo_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function woohoo_storage_concat_array( $var_name, $key, $value ) {
		global $WOOHOO_STORAGE;
		if ( ! isset( $WOOHOO_STORAGE[ $var_name ] ) ) {
			$WOOHOO_STORAGE[ $var_name ] = array();
		}
		if ( empty( $WOOHOO_STORAGE[ $var_name ][ $key ] ) ) {
			$WOOHOO_STORAGE[ $var_name ][ $key ] = '';
		}
		$WOOHOO_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'woohoo_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function woohoo_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $WOOHOO_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $WOOHOO_STORAGE[ $var_name ] ) ? $WOOHOO_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $WOOHOO_STORAGE[ $var_name ] ) ? $WOOHOO_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'woohoo_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function woohoo_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $WOOHOO_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $WOOHOO_STORAGE[ $var_name ]->$prop ) ? $WOOHOO_STORAGE[ $var_name ]->$prop : $default;
	}
}
