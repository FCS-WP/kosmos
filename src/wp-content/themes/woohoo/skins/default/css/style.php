...
if ( ! function_exists( 'woohoo_customizer_get_css' ) ) {

	function woohoo_customizer_get_css( $args = array() ) {
		...
		if ( $fonts ) {
			...
			$fonts        = woohoo_customizer_add_theme_fonts( $fonts );
			$rez          = array();
...
CSS;
			$rez          = apply_filters( 'woohoo_filter_get_css', $rez, array( 'fonts' => $fonts ) );
			$css['fonts'] = $rez['fonts'];
		}
		...
		if ( $vars ) {
			...
			$vars = woohoo_customizer_add_theme_vars( $vars );

			$rez         = array();
			$rez['vars'] = '';
			$rez         = apply_filters( 'woohoo_filter_get_css', $rez, array( 'vars' => $vars ) );
			$css['vars'] = $rez['vars'];
		}
		...
		//--------------------------------------
		if ( false !== $colors ) {
			$schemes = empty( $scheme ) ? array_keys( woohoo_get_sorted_schemes() ) : array( $scheme );
			if ( count( $schemes ) > 0 ) {
				$rez = array();
				foreach ( $schemes as $s ) {
					// Prepare colors
					if ( empty( $scheme ) ) {
						$colors = woohoo_get_scheme_colors( $s );
					}
					...
					$rez['colors']  = '';
					$rez            = apply_filters(
						'woohoo_filter_get_css', $rez, array(
							'colors' => $colors,
							'scheme' => $s,
						)
					);
		...
		return apply_filters( 'woohoo_filter_prepare_css', $css_str, $remove_spaces );
	}
}
