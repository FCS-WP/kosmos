...
?><div id="post-<?php the_ID(); ?>" <?php post_class( 'related_item post_format_' . esc_attr( $woohoo_post_format ) ); ?> data-post-id="<?php the_ID(); ?>">
	...
	woohoo_show_post_featured(
		array(
			'thumb_size'    => apply_filters( 'woohoo_filter_related_thumb_size', woohoo_get_thumb_size( (int) woohoo_get_theme_option( 'related_posts' ) == 1 ? 'big' : 'med' ) ),
		)
	);
	?>
	<div class="post_header entry-header">
		...
		if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
			?>
			<div class="post_meta">
				<a href="<?php echo esc_url( $woohoo_link ); ?>" class="post_meta_item post_date"><?php echo wp_kses_data( woohoo_get_date() ); ?></a>
			</div>
			<?php
		}
		?>
