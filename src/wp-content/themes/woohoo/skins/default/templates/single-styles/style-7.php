...
if ( apply_filters( 'woohoo_filter_single_post_header', is_singular( 'post' ) || is_singular( 'attachment' ) ) ) {
	...
	ob_end_clean();
	$woohoo_with_featured_image = woohoo_is_with_featured_image( $woohoo_post_header, array( 'with_gallery' ) );
	// Post title and meta
	ob_start();
	...
	if ( strpos( $woohoo_post_header, 'post_featured' ) !== false
		...
		<div class="post_header_wrap post_header_wrap_in_header post_header_wrap_style_<?php
            ...
            if ( $woohoo_with_featured_image ) {
                echo ' with_featured_image' . ( false && woohoo_get_theme_option( 'single_parallax' ) == 0 ? ' woohoo-full-height' : '' );
            }
		?>">
			<?php
