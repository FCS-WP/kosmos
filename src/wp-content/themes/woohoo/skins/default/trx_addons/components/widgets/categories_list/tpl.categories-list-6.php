...
// Widget body
$title_link_text = esc_html__('View All','woohoo');
if(isset($args['taxonomy']) && 'product_cat' == $args['taxonomy']){
	$title_link_text = esc_html__('Shop Now','woohoo');
}
				...
				?><div class="<?php echo esc_attr(trx_addons_get_column_class(1, $columns, $columns_tablet, $columns_mobile)); ?>"><?php
					...
					<div class="categories_list_item">
						...
						$src = empty($image)
							...
							: trx_addons_add_thumb_size($image, woohoo_get_thumb_size($columns > 2 ? 'medium-square' : 'huge'));
						$attr = trx_addons_getimagesize($src);
						?><img src="<?php echo esc_url($src); ?>" <?php if (!empty($attr[3])) trx_addons_show_layout($attr[3]); ?> alt="<?php esc_attr_e('Category image', 'woohoo'); ?>">
						</div><?php
						?>
						<div class="categories_list_info_wrap">
							<div class="categories_list_info">
								...
								<a href="<?php echo esc_url($cat_link); ?>" class="categories_link_more"><?php woohoo_show_layout($title_link_text); ?></a>
							</div>
						</div>
