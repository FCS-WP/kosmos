...
// SVG
$svg_bg_1 = trx_addons_get_svg_from_file(woohoo_get_file_dir('images/svg_bg_1.svg'));
$svg_bg_2 = trx_addons_get_svg_from_file(woohoo_get_file_dir('images/svg_bg_2.svg'));
		...
		if ( has_post_thumbnail() && $args['featured']=='image') {
			trx_addons_get_template_part('templates/tpl.featured.php',
											...
											apply_filters( 'trx_addons_filter_args_featured', array(
																...
																'thumb_size' => ! empty( $args['thumb_size'] )
																					? $args['thumb_size']
																					: woohoo_get_thumb_size('masonry-big'),
			...
			?>><?php
				if (!empty($svg)) {
					...
					?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="<?php esc_attr_e('Icon', 'woohoo'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
				}
			echo !empty($link) 
				? '</a>' 
				: '</span>';
			...
			?><img src="<?php echo esc_url($meta['image']); ?>" alt="<?php esc_attr_e('Icon', 'woohoo'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
			echo !empty($link) 
				? '</a>' 
				: '</span>';
		} else {
